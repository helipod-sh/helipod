...
**Em dashes. Remove all of them.** The em dash (—) is the single strongest tell of AI writing. Do not "reduce" them, eliminate them. A finished piece should contain zero. The same goes for the en dash (–) used as a sentence break. This is a hard rule, not a preference.

How to replace one depends on what it was doing:

- Joining two full thoughts → use a period and start a new sentence, or a semicolon if they're tightly linked.
- Tacking on an aside or afterthought → use a comma, or parentheses if it's truly parenthetical.
- Introducing a list, a definition, or a payoff → use a colon.
- Standing in for "to" in a range (pages 3—5) → use "to" or a hyphen (pages 3-5).
...
After: "The term is primarily promoted by Dutch institutions, not by the people themselves. You don't say "Netherlands, Europe" as an address. Yet this mislabeling continues, even in official documents."

After you rewrite, search the whole file for the character `—` (and `–`) and confirm the count is zero. A single one left behind undoes the effect.

The trap: don't swap an em dash for a comma and leave a comma splice ("The list updates, you write no polling code" is two sentences joined by a comma). If both sides can stand alone, use a period or colon, not a comma.
