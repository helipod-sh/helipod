...
import { defineAuth, consoleEmail, googleProvider, githubProvider } from "@helipod/auth";
import { defineScheduler } from "@helipod/scheduler";
import { defineWorkflow, workflow } from "@helipod/workflow";

// `@helipod/scheduler`/`@helipod/workflow` are opt-in like every other component (see
...
const auth = defineAuth({
  ...
  jwt: { issuers: [{ issuer: process.env.OIDC_ISSUER ?? "https://example.clerk.accounts.dev", audience: process.env.OIDC_AUDIENCE ?? "helipod" }] },
});

// A minimal illustrative workflow — the reference pattern real projects extend: a single
// `step.runQuery` against this project's own `whoami:get` (`helipod/whoami.ts`), referenced by its
