...
const fixtureSrc = join(here, "fixtures", "conventional-app", "helipod");
...
// `@helipod/*` imports resolve via Node's own ancestor walk. In a real end-user project
// (single package.json, single node_modules with @helipod/* installed from the registry)
...
// `packages/cli/node_modules/@helipod/*` (this package's own dependencies) actually has
...
// (ERR_MODULE_NOT_FOUND for @helipod/values) that has nothing to do with bundle-on-load —
 ...
 *  external @helipod/* imports resolve from packages/cli's own node_modules — see the
...
function codegenWith(runner: string): { ok: boolean; out: string } {
  ...
  const functionsDir = join(tmp, "helipod");
