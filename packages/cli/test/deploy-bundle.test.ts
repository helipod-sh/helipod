...
describe("resolveDeployOptions", () => {
  it("resolves --url + HELIPOD_ADMIN_KEY", async () => {
    const r = await resolveDeployOptions(["--url", "http://x:1"], { HELIPOD_ADMIN_KEY: "k" } as NodeJS.ProcessEnv);
  ...
  it("falls back to HELIPOD_DEPLOY_URL", async () => {
    const r = await resolveDeployOptions([], { HELIPOD_ADMIN_KEY: "k", HELIPOD_DEPLOY_URL: "http://y:2" } as NodeJS.ProcessEnv);
    expect(r).toMatchObject({ url: "http://y:2" });
  });
  it("errors on missing url", async () => {
    expect(await resolveDeployOptions([], { HELIPOD_ADMIN_KEY: "k" } as NodeJS.ProcessEnv)).toHaveProperty("error");
  });
  it("errors on missing/blank admin key", async () => {
    expect(await resolveDeployOptions(["--url", "http://x:1"], { HELIPOD_ADMIN_KEY: "  " } as NodeJS.ProcessEnv)).toHaveProperty("error");
  });
});
...
describe("packageApp", () => {
  ...
  beforeAll(() => {
    ...
    writeFileSync(join(dir, "schema.ts"), `import { defineSchema } from "@helipod/values";\nexport default defineSchema({});\nconst x: number = 1; void x;\n`);
    ...
    writeFileSync(join(dir, "_generated", "server.ts"), `export { query } from "@helipod/executor";\n`);
  ...
  it("transpiles every .ts (not .d.ts) preserving the tree, imports untouched", async () => {
    ...
    expect(byPath["schema.js"]).toMatch(/@helipod\/values/);
    expect(byPath["schema.js"]).not.toMatch(/: number/);
    expect(byPath["messages.js"]).toMatch(/\.\/_generated\/server/);
    expect(byPath["_generated/server.js"]).toMatch(/@helipod\/executor/);
  });
});
