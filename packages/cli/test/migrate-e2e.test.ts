 ...
 * (action-needed, not auto-fixed) and whose scaffolded `stackbase.config.ts` composes the real
 * `@stackbase/scheduler` (which itself needs `cron-parser`) — heavy infra irrelevant to what this
 * task is proving. `convex-app-basic` migrates to ONLY `@stackbase/*` + `./_generated/server`
...
beforeEach(() => {
  ...
  // The migrated schema.ts imports "@stackbase/values", and the regenerated
  // `_generated/server.ts` re-exports from "@stackbase/executor" — the temp dir lives outside
  // the workspace, so it has no node_modules of its own to resolve those bare specifiers from.
  // Symlink the whole @stackbase scope from the CLI package's own install, same precedent as
