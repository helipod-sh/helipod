 ...
 * placement hint (`HELIPOD_DO_LOCATION_HINT`). A real workerd `DurableObjectNamespace` is driven end
...
describe("createWorkerHandler — deployment location hint", () => {
  it("passes NO options bag when the hint env is unset (byte-identical to pre-hint)", async () => {
    const handler = createWorkerHandler("HELIPOD_DO");
    const { ns, calls } = fakeNamespace();
    const res = await handler.fetch(new Request("https://w.test/api/health"), { HELIPOD_DO: ns });
  ...
  it("threads a valid HELIPOD_DO_LOCATION_HINT into get(id, { locationHint })", async () => {
    const handler = createWorkerHandler("HELIPOD_DO");
    const { ns, calls } = fakeNamespace();
    const res = await handler.fetch(new Request("https://w.test/api/health"), {
      HELIPOD_DO: ns,
      HELIPOD_DO_LOCATION_HINT: "enam",
    });
    expect(res.status).toBe(200);
  ...
  it("rejects an INVALID hint with a loud 500 at the edge, never reaching a DO", async () => {
    ...
    const res = await handler.fetch(new Request("https://w.test/api/health"), {
      HELIPOD_DO: ns,
      HELIPOD_DO_LOCATION_HINT: "atlantis",
    });
    expect(res.status).toBe(500);
    expect((await res.json()).error).toContain("HELIPOD_DO_LOCATION_HINT");
