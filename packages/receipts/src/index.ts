/**
 * `@stackbase/receipts` — the recurring driver seam for the Receipted Outbox's server-side TTL
 ...
 * `sweepExpiredClientMutations`) lives directly on `DocStore` (`@stackbase/docstore` + its
 * `docstore-sqlite`/`docstore-postgres` implementations) — this package is just the periodic sweep
 * driver on top, mirroring `@stackbase/storage`'s `storageReaper`.
 */
