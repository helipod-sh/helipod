...
```bash
bun ../../packages/cli/dist/bin.js dev --dir stackbase --web web --port 3210
```
...
2. It watches the `stackbase/` folder (`--dir stackbase`) for function and schema changes.
...
If you only changed a function in `stackbase/` you don't need to do anything else. The watch loop
