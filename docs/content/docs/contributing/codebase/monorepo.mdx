...
Stackbase lives in one repository. That's not an accident: it's the same "everything in one place, split into small pieces" idea as the reactive engine itself.
...
`apps/dashboard` is the live data browser, logs viewer, and function runner you get with every `stackbase dev`/`serve`. `examples/` (`chat`, `auth-demo`, `offline-demo`, `optimistic-demo`) double as end-to-end integration tests that run against the real CLI. A regression in wiring the pieces together shows up there, not just in unit tests.
...
See [Architecture: runtimes](/docs/contributing/architecture/runtimes) for how this plays out across Tier 0 (single binary) and beyond.
...
If a name isn't in this table, it's either a leaf adapter (`docstore-postgres`, `blobstore-s3`, ...) whose job is obvious from its name, test-only scaffolding (`test`), or a tooling integration (`vite`, the `@stackbase/vite` plugin that boots the stackbase dev backend alongside a Vite dev server).
...
If you want to build your own, [Building a custom component](/docs/contributing/extending/custom-component) walks through the seams (`schema`, `context`, `modules`, `driver`, `boot`, `httpRoutes`) that `defineComponent` gives you. They're the same ones auth and friends use.
