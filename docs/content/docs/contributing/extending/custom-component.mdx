...
This is a how-to for building your own component: a package shaped like `@stackbase/scheduler` or
`@stackbase/auth` that a project opts into from its `stackbase.config.ts`. It assumes you already
...
as a couple of tables and a background loop (`@stackbase/triggers`), or as full-featured as tables, a
facade, an action-mode facade, HTTP routes, and a boot step (`@stackbase/auth`). The six components
...
`stackbase.config.ts`. Nothing is auto-installed, and nothing about the engine itself needs to change
...
or a client is meant to invoke directly. `@stackbase/scheduler`'s own dispatch loop, for example,
...
`@stackbase/authz`'s permission checks) only ever need to read. If your facade needs to write (insert
...
- `serverExports: string[]` lets you re-export a helper alongside `query`/`mutation`/`action` from
  `_generated/server.ts`, sourced from the same `contextType.import` path. `@stackbase/scheduler` sets
...
- **`readLog({ afterTs, tables?, limit? })`**: reads committed changes from the underlying change log
  ...
  component (like `@stackbase/triggers`) is built on, described further below. Each returned change in
...
Not every driver needs a writable facade. `@stackbase/triggers` has no `context` at all: you configure
it declaratively (`{ table: { handler } }` in `stackbase.config.ts`), and its driver is just a cursor
...
`@stackbase/scheduler` uses it to read an app's declared `cronJobs()` schedule and write matching rows
...
<TypeTable
  type={{
    requires: {
      type: 'string[]',
      description: 'Names other components yours needs composed alongside it. @stackbase/workflow declares requires: ["scheduler"] because it dispatches its steps through the scheduler\'s job queue. The composing project gets a clear compose-time error (component "widget" requires "scheduler", which is not enabled) instead of a confusing runtime failure the first time your facade is touched.',
    },
    config: {
    ...
    policies: {
      ...
      description: 'How a component like @stackbase/authz attaches row-level read/write rules to an app\'s own tables, and contributes fields (like the caller\'s identity) into every policy\'s evaluation context. Most components never touch this pair at all.',
    },
  }}
/>
...
listing it in `stackbase.config.ts`:
...
This composed set is fixed at boot. Adding or removing a component from `stackbase.config.ts` needs a
restart (`stackbase serve` again, or a fresh `stackbase build`); there's no live "activate this
component" call. That's a deliberate, narrower boundary than `stackbase deploy`'s live hot-swap of
