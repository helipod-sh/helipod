...
Each reconnect attempt waits `reconnectDelayMs(attempt, initialBackoffMs, maxBackoffMs)`. The schedule
uses equal jitter: half the exponential delay, plus up to another half at random. This never produces
a zero wait, which avoids a thundering-herd reconnect storm the instant a server comes back, and it's
...
network blip on load), the transport queues frames until a socket does open. When one finally does,
it runs the same reopen sequence (`SetAuth` replay, resubscribe, flush) as a mid-session reconnect,
even though technically no socket had "reopened" yet. From the app's point of view there is exactly
one code path for "the client now has a live connection and needs to rebuild state from what it
remembers": first connect after a failed attempt, and every subsequent reconnect.
...
here too. The underlying client methods, `client.pendingMutations()` and `client.pendingSummary()`
...
<Callout type="warn" title="Migrating from Convex?">

This is a deliberate divergence from `convex-js`, whose mutation promise resolves earlier, at what
Stackbase calls the optimistic reconciliation "gate": the moment the client's own reactive feed has
observed the write, not merely been told it succeeded. If you relied on that earlier resolution
timing (for example reading freshly-committed state synchronously right after `await mutation(...)`),
note that here the promise's resolution and the query re-render triggered by the same write can
arrive in either order relative to each other. See
