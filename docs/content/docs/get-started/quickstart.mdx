...
<Callout title="Before you start">
  ...
  `stackbase/`. That's just the default name (everything you import is `@stackbase/*`), and you can
...
Create `stackbase/schema.ts`. It lists your tables and the shape of each row:
...
`stackbase dev` generates the typed `stackbase/_generated/` helpers, starts the engine on a local SQLite
...
The admin key is a fresh random value on each run (unless you set `STACKBASE_ADMIN_KEY` yourself).
Yours will differ.
...
| `--dir` | `stackbase` | Function directory |
...
<Accordion title="What's in stackbase/_generated/">

After the first run, `stackbase/_generated/` holds typed helpers derived from your schema and functions:
