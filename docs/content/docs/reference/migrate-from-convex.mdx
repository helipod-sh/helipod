...
[What is stackbase?](/docs/get-started/what-is-stackbase)). `stackbase migrate` also renames your
app's function directory from `convex/` to stackbase's own default, `stackbase/`, as part of the
same run. Migrating moves your app onto the native import surface; it does not make stackbase
...
- `query`/`mutation`/`action` still come from `./_generated/server`. Your function bodies don't
  move.
- `schema.ts` keeps its shape (it just relocates from `convex/` to `stackbase/` along with the rest
...
directory (`--dir`, default `convex`) differs from where stackbase itself would put it
(`stackbase/` by default, or your project's own `functionsDir` in `stackbase.config.ts`), the whole
...
`loadFunctionsDir → push → writeGenerated` pipeline `stackbase codegen` uses.
...
```ts title="stackbase/schema.ts (after migrate)"
...
- `convex/`: renamed to stackbase/. **Fix:** Your backend functions now live in stackbase/. Imports inside that folder are relative and did not change.
- `stackbase/schema.ts:1`: import "convex/server" (schema). **Fix:** rewritten to "@stackbase/values"
- `stackbase/schema.ts:2`: import "convex/values". **Fix:** rewritten to "@stackbase/values"
- `stackbase/messages.ts:1`: import "convex/react". **Fix:** rewritten to "@stackbase/client/react"
- `stackbase/http.ts:1`: import "convex/server" (http). **Fix:** rewritten to "./_generated/server"

## Action needed (3)

- `stackbase/messages.ts:14`: .withIndex(...) query. **Fix:** Stackbase has no .withIndex, use ctx.db.query(table, "index").eq(f, v).gte(f, v).order("asc"|"desc").collect()
- `stackbase/messages.ts:22`: ctx.db.patch(...). **Fix:** Stackbase has no patch, read the doc, spread-merge, ctx.db.replace(id, { ...doc, ...changes })
- `stackbase/crons.ts:1`: Convex crons (cronJobs). **Fix:** Compose defineScheduler() in stackbase.config.ts and use cronJobs() from "@stackbase/scheduler"

## Unsupported (1)

- `stackbase/search.ts:9`: vector/search index. **Fix:** search/vector is not yet supported in Stackbase (see roadmap)
```
...
`stackbase migrate` rewrites only your app's own function-directory source (renamed from `convex/`
to `stackbase/` as part of the same run). It does not touch `convex-test` call sites. Switch those
to [`@stackbase/test`](/docs/reference/testing) yourself.
...
stackbase's own default function directory is `stackbase/`, not `convex/`; `convex/` was never the
target, only where a Convex app starts from, and `stackbase migrate` renames it as part of the same
run. There's no alias path back to `convex/*` imports to maintain: once migrated, you're simply
writing a native stackbase app.
