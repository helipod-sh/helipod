...
<Accordion title="A Bun.SQL Postgres driver">

The shipped `@stackbase/docstore-postgres` implementation (`NodePgClient`) is built on the
[`pg`](https://node-postgres.com/) driver, which runs on both Node and Bun. The interface
(`PgClient`: `query`/`transaction`/`acquireWriterLock`/`close`) is deliberately shaped so a
`Bun.SQL`-backed client could drop in with zero changes to `PostgresDocStore`. It's a reserved,
seam-swappable fast-follow, but it doesn't exist yet.
...
<Accordion title="Some Tier-3 tail items">
...
MinIO. But multi-shard-node support, a dedicated replica-serve mode, a standalone GC driver with its
own fencing, and a reshard tool remain unbuilt tail items on that arc.
...
reference it's cloning. See [How fast is it?](#how-fast-is-it-measured-numbers) below for the full
...
[Reproducing these numbers](#reproducing-these-numbers) for how to reproduce any of them. The
running theme is honest measurement: same conditions on both sides of a comparison, caveats
...
of 10,000" cell from **6.72 ms p50 to 0.24 ms p50** (a 96.4% drop). That's the fix that keeps write
cost independent of how many other subscriptions exist on a node.
...
| single-node sharding (Postgres, in-process) | **~2.6 to 2.85× at 8 shards**, sub-linear with a knee around 4 shards (shared WAL) |
...
| axis | result |
|---|---|
| online round-trip latency, adapter on vs off | **~0** (0.121ms vs 0.119ms p50) |
...
|---|---:|---:|
| fingerprints OFF | 310,882 | baseline |
| fingerprints ON | 2,052 | **99.3% smaller** |
...
One sync node holds 10,000 concurrent subscribed connections cleanly (all connected, zero failed
sockets) at 7.69 KB/connection and 0.4% idle CPU. Under hot fan-out (one hot query, all 10,000
subscribers), push latency grows roughly linearly with subscriber count: about 19 µs of added
write-to-observed latency per additional subscriber, saturating most of one core by N=10,000. This
is the honest capacity edge of the current single-threaded per-subscriber send path, not a defect.
See the connections benchmark for the full shape.

### Fleet cross-node behavior

| metric | result |
|---|---|
| cross-node push latency (writer commits, a different node's subscriber receives it) | **~15 to 16 ms p50**, flat across fleet width K=1/2/4 |
| RSS/connection tax for running in fleet mode at all | none found, same curve as single-node |
| idle CPU tax for fleet mode | **~2 to 3 percentage points**, flat regardless of fleet width |
...
2,000 concurrent subscribers. It survives a full kill-all reconnect storm at 100%
`QueryUnchanged` resume, using under 12% of its CPU budget.
...
Every number above carries machine and substrate caveats in its source report. Docker Desktop on
...
Yes, via `@stackbase/runtime-cloudflare`, in one of two genuinely different architectures. Neither
is wired into `stackbase deploy`'s automatic push flow. Both are a `wrangler deploy` of a small
entrypoint that imports a stackbase host package.
