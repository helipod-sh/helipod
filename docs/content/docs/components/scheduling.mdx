...
non-negative (`Math.max(0, delayMs)`), so `runAfter(-100, ...)` just means "as soon as possible,"
not an error.
...
`runAfter`/`runAt` are the common case, and they only accept a delay or a timestamp. For the rest of
the options, call the lower-level `enqueue` method directly:
...
<TypeTable
  type={{
    runAfter: {
      type: 'number',
      description: 'Relative delay in ms. Mutually exclusive in practice with runAt: whichever is set wins, and omitting both means run as soon as possible.',
    },
    runAt: {
      type: 'number',
      description: 'Absolute epoch-ms fire time.',
    },
    idempotencyKey: {
      type: 'string',
    ...
    retry: {
      ...
      description: 'Overrides the default retry ceiling (4) for this job.',
    },
    name: {
      type: 'string',
...
- **Actions** have arbitrary external side effects (an HTTP call, a charge), so re-running one that
  ...
  but a crash mid-flight, meaning the process died while the action was running before it could
  report success or failure, dead-letters the job (`state: "failed"`) instead of blindly
  re-dispatching it.
...
run at all. In practice, "parent" chains today come from components (like `@stackbase/workflow`,
whose steps are chained this way) that explicitly link a `parentId`. A plain
`ctx.scheduler.runAfter` call made from inside another job's own handler does not automatically
become that job's child.
...
deterministic error (for example a schema-violating write) skips straight to dead-lettering instead
...
- **Mutation** jobs are reclaimed back to `pending` with `nextTs: now()` (immediate, no backoff,
  ...
  mutation is deterministic and transactional, replaying it is safe.
- **Action** jobs are reclaimed straight to `state: "failed"` instead. An expired lease means
  there's no way to know whether the action's side effects already ran, so retrying could double-run
  them. `onComplete`, if set, fires with `{ kind: "failed" }`, so a caller relying on the callback
  (like a workflow step) still observes the terminal outcome rather than hanging forever.

Put another way: scheduled mutations are effectively at-least-once at the granularity of the whole
job, since a crash-recovered retry always eventually lands the same deterministic write. Scheduled
...
[standalone binary](/docs/deploy/standalone-binary)).
