...
`defineWorkflow()`, and so on). The factory validates its options and hands back the object. Nothing
executes yet.

Composition (reading these objects and wiring them into the running engine) happens once, at boot,
in `packages/component`'s `composeComponents`.
...
Every field is optional except `name`, `schema`, and `modules`. A component can be as small as "a
couple of tables and a background driver" (`@stackbase/triggers`) or as large as "tables, a facade,
an action-mode facade, HTTP routes, and a boot step" (`@stackbase/auth`). Here's what each
contribution does.

### `schema`: namespaced tables

A component's `schema` is an ordinary `SchemaDefinition`, exactly like your app's `schema.ts`. At
compose time, every component table is namespaced under the component's name: `@stackbase/scheduler`'s
`jobs` table becomes `scheduler/jobs`, and `@stackbase/auth`'s `sessions` becomes `auth/sessions`.
That means a component table can never collide with one of your app's own tables, and two composed
components can never collide with each other either.

Your app's own tables stay bare (`messages`, not `app/messages`). The app is "component zero" in the
compose step, the one namespace that isn't prefixed.

Table numbers are allocated once, in the order components are composed (after topological sorting by
`requires`, see below), by `composeTables` in `packages/component/src/compose.ts`. A duplicate table
name (the same full name from two sources) is a compose-time error, not a silent overwrite.

### `modules`: namespaced functions

A component's own registered functions (queries, mutations, actions, and httpActions it needs
internally) are namespaced the same way, but with `:` instead of `/`. `@stackbase/scheduler`'s
internal dispatch mutation becomes `scheduler:_dispatch`, and `@stackbase/auth`'s sign-up mutation
becomes `auth:signUp`.
...
internal functions live safely in their own namespace. `composeModules` rejects composing a component
whose `name` collides with one of your app's own top-level module prefixes.
...
query, mutation, and action in your app.

`cctx.db` inside that builder is read-only by default. Set `contextWrite: true` (as
`@stackbase/scheduler` and `@stackbase/auth` do) to let the facade write to the component's own
namespaced tables during a mutation call. It stays read-only during a query, since queries never
write.

`contextType` is separate from `context`. It isn't code: it's the `{ import, type }` pair codegen
needs to type `ctx[name]` correctly in the generated `_generated/server.ts` (for example `{ import:
"@stackbase/scheduler", type: "SchedulerContext" }`). Declaring `contextType` without `context` is a
compose-time error (`defineComponent` rejects it), because a typed-but-undefined `ctx.foo` is a worse
failure mode than no type at all. The two are enforced to travel together.

### `buildAction`: the action-mode facade

Actions don't have `ctx.db`. They run outside the transaction, non-deterministically, so a component
whose facade is used from actions needs a second implementation with the same method signatures but
no direct table access.

`buildAction: (api: ActionApi) => object` builds that version by delegating to the component's own
internal mutations and queries via `api.runMutation`/`api.runQuery`, instead of touching `cctx.db`
directly. `@stackbase/scheduler`'s `schedulerActionContext` and `@stackbase/workflow`'s
`workflowActionContext` both follow this pattern: an action-mode facade delegates to an internal
mutation with the same effect.

This is why `ctx.scheduler.runAfter(...)` and `ctx.workflow.start(...)` work identically whether you
call them from a mutation or from an action. Same object shape, two different implementations
underneath.

### `serverExports`: codegen re-exports

Some components want to export a helper alongside `query`, `mutation`, and `action` from
...
`policies` lets a component declare `TablePolicy` read/write rules for your app's own tables. This is
how `@stackbase/authz` attaches RBAC/ReBAC row policies without your `schema.ts` knowing anything
about authorization.

`policyContext` lets a component contribute fields into every policy rule's evaluation context
(`@stackbase/authz` adds `{ auth }` so a policy can check the caller's identity). A policy declared
for a table that doesn't exist, or a duplicate policy for the same table from two components, is a
compose-time error.

### `boot`: one-time startup work

`boot: (ctx: BootContext) => Promise<void>` runs once per process start, namespaced and
non-user-triggered. `@stackbase/scheduler` uses it to reconcile `crons.ts`'s declared schedules into
the `jobs`/`crons` tables; `@stackbase/authz` uses it to rebuild an authorization index
...
It's substantial enough to deserve its own section: see [The driver seam](#the-driver-seam) below.

### `httpRoutes`: reserved-path routes

`httpRoutes: ComponentHttpRoute[]` lets a component mount routes at the engine's reserved
`/api/*`/`/_*` namespace. It's the same mechanism the engine's own built-in routes use, generalized
so a component gets it too. `@stackbase/auth` uses this for its OAuth start/callback endpoints
(`/api/auth/oauth/`), and `@stackbase/notifications` uses it for provider delivery webhooks
(`/api/notifications/webhooks/`). See [Reserved HTTP routes](#reserved-http-routes) below.

### `requires`: declared dependencies

`requires: string[]` names other components this one needs composed alongside it. `@stackbase/workflow`
declares `requires: ["scheduler"]` because it dispatches workflow steps through the scheduler's job
queue. `@stackbase/authz` declares `requires: ["auth"]` because permission checks resolve the
caller's identity through `ctx.auth`. See [Dependency ordering](#dependency-ordering) below.
...
`.components` off the default export. If the file is missing, the project composes zero components
(`{ components: [] }`) and behaves as pure core engine.
...
prefix (`scheduler:_dispatch`), and its `ctx[name]` key (`ctx.scheduler`), all three at once. That's
