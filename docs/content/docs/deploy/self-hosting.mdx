...
`stackbase serve`, your app's `convex/` folder is mounted in, and its SQLite database sits on a
...
You need a `convex/` directory with **committed `_generated/`**. `serve` fails fast without it, as
...
```mermaid
flowchart LR
  Convex["./convex (read-only)"] -->|"bind mount"| Container["stackbase:latest running serve"]
...
That's fine for code living inside a workspace package. But your bind-mounted `/app/convex` is not
...
`@stackbase/*` name straight into `/app/node_modules/`, so a mounted (or baked-in) `convex/`
...
The default compose file bind-mounts `convex/` at run time. That's convenient for local
...
separate `convex/` directory around, build a small wrapper `Dockerfile` on top of
...
`convex/` resolves its `@stackbase/*` imports exactly the same way a mounted one does. No extra
...
`stackbase serve --dir ./convex --data /app/data/db.sqlite`. Fly terminates TLS at its edge.
