...
From your app's root (where `stackbase/` lives):
...
1. **Loads your project.** It resolves `stackbase.config.ts` (component composition), then reads
   `stackbase/schema.ts` and every function file under `stackbase/`.
2. **Generates `stackbase/_generated/`**: the typed `api`/`Doc`/`Id` helpers your functions and client
   import.
...
5. **Starts watching `stackbase/`** for changes and hot-reloads your functions on every save (see
...
| `--dir <path>` | `stackbase` | Where your schema and functions live. |
...
`stackbase dev` watches `stackbase/` (recursively) via a filesystem watch, ignoring anything under
`stackbase/_generated/` so codegen's own writes don't re-trigger themselves. On every change, after a
short debounce, this is the loop:

```mermaid
flowchart LR
  Edit["You save a file in stackbase/"] --> Debounce["Watcher debounces"]
...
2. Re-writes `stackbase/_generated/` with the freshly generated types.
3. Calls `runtime.setModules(...)` with the new function map. This replaces the module map
   ...
   reload. They aren't part of your `stackbase/` source, so they'd otherwise vanish on the first save.
...
```bash
stackbase codegen
stackbase codegen --dir stackbase   # if your functions aren't in ./stackbase
```

This regenerates `stackbase/_generated/` and exits: no server, no watch loop. `stackbase dev` does this
...
- [Self-hosting with Docker](/docs/deploy/self-hosting): `stackbase serve`, the production
  ...
  key-less).
- [Deploying without a restart](/docs/deploy/deploy-and-build): `stackbase deploy`, for pushing a
  schema and function change to an already-running `serve` without hot-reload's dev-only watch loop.
- [Vite plugin](/docs/deploy/vite-plugin): a Vite app can run `stackbase dev` for you as part of
  its own dev server.
