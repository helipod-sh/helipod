...
- **[Write Sharding — Research & the Fenced Frontier Verdict (proposed)](./architecture/write-sharding-research.md)** — the writes axis: 10-agent adversarial research (3 evidence, 3 opposing designs, 3 critics, 1 judge; corpus in [research/write-sharding/](./research/write-sharding/)). Verdict: **Fenced Frontier** — one global ts line, per-shard parallel OCC writers, visibility = min per-shard frontier, with lease=fence=frontier as one commit-atomic Postgres row (lock-serialized eviction closes the skipped-ts class). Slices B1 (fleet hardening, valuable alone) → B4 (group commit).
