...
- **B5 — design-doc only — DELIVERED**: object-storage substrate mapping + offline reshard tool,
  as a [design record](../research/write-sharding/b5-reshard-and-object-storage.md) (no committed
  code, per the verdict's slice plan). Part 1 specifies `stackbase fleet reshard --shards M` — an
  offline-only tool that, because `documents.shard_id`/`indexes.shard_id` are write-only decoration
  (routing always recomputes `shardIdForKeyValue(key, N)`), is **not a data migration**: it moves no
  rows, only updates the persist-once `fleet:numShards` global and re-seeds the `shard_leases` row
  set via the shared `frontierSeedExpr`. Part 2 maps every shipped mechanism onto an S3-class
  object-storage substrate (lease=fence=frontier → one CAS-updated manifest per shard) and lists the
  protocol decisions already made — scalar store-allocated ts, one-row frontier, batch-shaped guard,
  epoch-as-fence, `commitMeta` idempotency — that keep that future (unscheduled) tier reachable. **The
  build arc (B1 → B4) is complete; B5 closes it as a design record.**
