...
- **B5 — Design-doc level (not committed code).** Object-storage substrate (per-shard segment logs, CAS frontier manifests with epoch as fence, lease-granted ts ranges for the sequencer seam); offline NUM_SHARDS reshard tool. **DELIVERED as a design record → [`b5-reshard-and-object-storage.md`](./b5-reshard-and-object-storage.md). The arc is complete.**
