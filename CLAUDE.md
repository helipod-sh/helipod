...
Honestly deferred (seams reserved, not built): true V8-isolate global sandboxing (the inline executor runs in-process; the syscall ABI is isolate-ready) · optimistic updates + full version-gap resync in the client · range-precise invalidation (table-level ships; ranges already recorded) · Tier 2 distributed sharding/sync fleet · auth · file storage · search/vector · scheduled functions/crons · dashboard UI. These are the remaining build-order slices (#2–#6).
