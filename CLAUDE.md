...
3. Auth
4. File storage
5. Actions + scheduled functions / crons (side effects that run *outside* the transaction) — **both halves ✅ shipped**: scheduled functions/crons (`@stackbase/scheduler`) and actions themselves (see What works above); `httpAction` + the public HTTP router (webhooks) remain deferred
