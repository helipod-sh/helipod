...
```ts
import { HelipodClient, webSocketTransport, indexedDBOutbox } from "@helipod/client";
...
It keeps everything in one IndexedDB database (`helipod-outbox`), on purpose: the shared
...
`@helipod/client/outbox-fs` subpath, so browser bundles never see its `node:*` imports:
...
**One `clientId` per tab session**, minted once at `HelipodClient` construction and never reused
...
instead backs off (the same jittered-exponential shape `@helipod/scheduler` uses for its own
...
built for a context with no `HelipodClient` at all, most concretely a Service Worker's one-shot
Background Sync handler:

```ts
// sw.ts, inside your Service Worker
import { drainOutboxOnce } from "@helipod/client";

self.addEventListener("sync", (event) => {
  if (event.tag === "helipod-outbox-drain") {
    event.waitUntil(
      drainOutboxOnce({
...
`helipod codegen` emits `_generated/ids.ts` with a typed `mintId<T extends TableNames>(table: T):
...
Helipod has **no merge and no CRDT layer**, offline or online. A queued mutation, when it
...
- [Client SDK](/docs/client/client-sdk): constructing `HelipodClient`, transports, reconnection,
