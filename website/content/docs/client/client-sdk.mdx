...
`HelipodClient` itself never touches the network directly. Every method (`subscribe`, `query`,
...
`webSocketTransport(url, options?)` is for a real deployment, talking to `helipod dev`/`helipod
...
[`@helipod/notifications`](/docs/components/notifications)'s reactive in-app inbox. No per-app
