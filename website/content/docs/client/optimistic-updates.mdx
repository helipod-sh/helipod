...
import type { OptimisticLocalStore } from "@helipod/client";
...
This is an easy mistake to make. The store's shape is designed so the deterministic path is also the
easy path, not so the impure path is blocked by a lint rule. There isn't one.
...
<Callout type="info" title="What the resolved promise guarantees">

Resolving at commit rather than at the later supersede gate is deliberate. Gate-time resolution has
sharp edges this protocol doesn't want: a transport drop can turn a mutation that did commit into a
promise that rejects, and a lost gating frame with no follow-on traffic can leave a promise hanging
forever.

So the guarantee here is "committed," not "your optimistic guess has been definitively superseded."
Those are almost always indistinguishable in practice, but not quite always. See the two residuals
below. If you're porting code that relies on await-then-local-cache-read semantics, the
[migration guide](/docs/reference/migrate-from-convex) covers the difference.
