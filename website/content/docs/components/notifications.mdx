...
Think of a notification as just another row in a table. `@helipod/notifications` is an opt-in
...
- [Enabling it](#enabling-it) and [Sending](#sending): the core path, `SendArgs`, at-most-once keys
- [Channels and providers](#channels-and-providers): Resend, Twilio, writing your own, and
  [multi-provider fallback](#multi-provider-fallback)
- [Push](#push): device tokens, Expo/FCM/APNs, push non-goals
- [The reactive in-app inbox](#the-reactive-in-app-inbox): `useNotifications` and `<Inbox>`
- [Preferences and critical categories](#preferences-and-critical-categories) and [Topics](#topics)
- [Delivery reliability](#delivery-reliability): retries, crash reclaim, delivery webhooks
- [Email digest](#email-digest), [Auth unification](#auth-unification),
  [the driver](#how-delivery-actually-runs-the-driver), [schema reference](#schema-reference)
...
Compose the component in `helipod.config.ts`, like any other opt-in component:
...
different, send-focused surface (`send`, `sendNow`, `sendToTopic`, and nothing else). See
...
(`NotificationsActionContext`) is exactly three methods: `send`, `sendNow`, and `sendToTopic`, the
sends that need (or are allowed) to do work outside a transaction.

Everything else (`setPreference`, `subscribe`, `unsubscribe`, `registerPushToken`,
`unregisterPushToken`, `identity()`) exists only on the mutation facade. Calling one of them from
an action is a runtime TypeError, not a silent no-op. Do that work in a mutation the action invokes
via `ctx.runMutation` instead. `ctx.notifications.identity()` (mutation facade) returns the raw
ambient caller token (the fallback identity `callerId` uses when `@helipod/auth` isn't composed).
Most app code never needs it directly.

#### `sendNow`: synchronous delivery from an action

```ts title="helipod/blast.ts"
...
Email, SMS, and in-app are covered here. Push has enough of its own shape, multi-provider fan-out
and a device-token registry, that it gets [its own section](#push) below.
...
```ts
import type { EmailProvider, EmailMessage, SendResult } from "@helipod/notifications";
import { NotificationSendError } from "@helipod/notifications";
...
```ts title="helipod.config.ts"
import { defineConfig } from "@helipod/component";
import { defineNotifications, expoPush, fcmPush, apnsPush } from "@helipod/notifications";
...
<Accordion title="Provider auth and adapter details (Expo / FCM / APNs)">
...
```ts
import { registerForPush, unregisterForPush } from "@helipod/client/react";
...
```ts
import type { PushProvider, PushMessage, PushSendResult } from "@helipod/notifications";
...
```tsx
import { useNotifications } from "@helipod/client/react";
...
With `@helipod/auth` composed (or an upstream token-verifying proxy), the resolved id is
...
you write, or a composed component like `@helipod/auth`, see
...
```tsx
import { useNotificationPreferences } from "@helipod/client/react";
...
defineNotifications({
  ...
  retry: {
    ...
    initialBackoffMs: 250,   // backoff constant: the FIRST retry waits 250 * 2 ** 2 = 1000ms, jittered
    base: 2,                 // exponential multiplier
  },
});
```

The delay for the Nth retry is `initialBackoffMs * base ** (N + 1)`, jittered to 50-100%. With the
defaults, the first retry waits 500ms to 1,000ms (not ~250ms), the second 1s to 2s, and so on.
...
Point your provider's delivery webhook (Resend, Twilio) at your deployment and Helipod verifies
...
`WEBHOOK_PREFIX`, e.g. `https://your-host/api/notifications/webhooks/email` for Resend). What's
actually registered is one prefix route (`/api/notifications/webhooks/`), and only if at least one
configured channel's primary provider declares a `webhook`, so a project using only
`consoleEmail()`/`consoleSms()` never exposes it. Whether a specific `:channel` accepts events is
then checked per request against that channel's own configured provider. Configure the URL in the
...
`https://…` URL configured in its console, but Helipod serves plain HTTP behind your
...
Compose **both** `@helipod/auth` and `@helipod/notifications` in the same
`helipod.config.ts`, and auth's own transactional emails (verification, password reset, magic
...
- **Graceful fallback.** `@helipod/auth` does not depend on `@helipod/notifications` as a
...
<Accordion title="Table-by-table reference">
