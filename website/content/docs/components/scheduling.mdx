...
Think of `@helipod/scheduler` as cron built into your database, not bolted on beside it. A
scheduled job is a row in a `jobs` table. Running it later, once, at an exact time, or on a
recurring cadence, works the same as any other write in helipod: durably, with nothing polling in
the background.
...
It's the foundation two other components build on. [`@helipod/workflow`](/docs/components/workflows)
dispatches every step through the scheduler's own job queue, and both `@helipod/triggers` and
`@helipod/notifications` reuse the same recurring-driver seam for their own background loops.
...
`@helipod/workflow` schedules every step), and its option surface is narrower than the type
suggests.
...
This is also the primitive other components build on. `@helipod/workflow` schedules every step
...
path (which is how `@helipod/workflow` threads a step's declared `maxAttempts` through) is your
...
`@helipod/workflow` cancels its step jobs directly via `scheduler.cancel` rather than chaining
...
[`@helipod/workflow`](/docs/components/workflows) is built on: every step dispatch is a scheduled
job whose `onComplete` advances the workflow's own journal.

## Recurring jobs: `cronJobs()`

An app's `crons.ts` declares recurring work:
