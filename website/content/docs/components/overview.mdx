---
title: Components
description: Opt-in capabilities composed in helipod.config.ts.
---
...
A project composes exactly the components it wants in `helipod.config.ts`. Nothing is installed
...
```mermaid
flowchart LR
  Config["helipod.config.ts"] --> Compose["composeComponents\n(topological sort by requires)"]
...
You declare which components are active in a single `helipod.config.ts` at your project root,
...
(Adapted from `examples/auth-demo/helipod.config.ts`, the reference pattern real projects copy
from. `examples/chat/helipod.config.ts` shows the smallest possible case: a single
...
project. `packages/cli/src/load-config.ts` just imports `helipod.config.ts` (or
`helipod.config.js`, whichever exists) and reads `.components` off the default export. If neither
...
configuration `helipod deploy` reads (see [Deploy & build](/docs/deploy/deploy-and-build)).
`@helipod/component` also exports an `env(name, fallback?)` helper for referencing environment
...
|---|---|---|---|---|---|
| Auth | `@helipod/auth` | (none) | `ctx.auth` | (none) | `/api/auth/oauth/` (if `oauth` configured) |
| Authorization | `@helipod/authz` | `auth` | `ctx.authz` | (none) | (none) |
| Scheduling | `@helipod/scheduler` | (none) | `ctx.scheduler` | dispatch loop | (none) |
| Workflows | `@helipod/workflow` | `scheduler` | `ctx.workflow` | (via scheduler) | (none) |
| Triggers | `@helipod/triggers` | (none) | (none) | log-cursor loop | (none) |
| Notifications | `@helipod/notifications` | (none) | `ctx.notifications` | delivery sweep | `/api/notifications/webhooks/` (if a webhook provider is configured) |
...
true`, as `@helipod/scheduler`, `@helipod/workflow`, and `@helipod/notifications` do. `boot`
...
whichever subset it needs: `@helipod/authz` composes tables, a facade, policies, and a boot step;
`@helipod/scheduler` adds a driver, an action-mode facade, and a codegen re-export on top.
...
`helipod deploy` (see [Deploy & build](/docs/deploy/deploy-and-build)) can push a new version of
your app's own functions and an additive schema change onto an already-running `helipod serve`,
atomically, with no restart. But the *set* of composed components is decided once, at process boot,
from `helipod.config.ts`, and baked into the running deployment's table numbers, module map,
context providers, and drivers.

Adding `defineNotifications(...)` to your config, or removing `defineTriggers(...)`, requires
restarting the process (`helipod serve` again, or a redeploy of a `helipod build` binary).
There's no live "activate this component" admin call.

</Callout>

This mirrors the same boundary `helipod deploy`'s additive-schema gate draws for your own
...
request: the scheduler's job-dispatch loop, `@helipod/triggers`' cursor over the MVCC log,
`@helipod/notifications`' delivery sweep and retry/backoff loop.
...
- **`readLog({ afterTs, tables?, limit? })`**: reads committed changes from the MVCC log after a
  timestamp, in ascending order. This is the durable change-feed seam `@helipod/triggers` is built
...
- **`functionKind?(path)`** *(optional)*: resolves a registered path's real kind
  ...
  at start time (`@helipod/triggers`' `handler` option) and fail with an instructive "unknown path"
...
`@helipod/auth` mounts `GET`/`POST /api/auth/oauth/` (GET for query-mode OAuth callbacks, POST for
Apple's `form_post` callback: same handler, two methods, no overlap since they're keyed per method).
`@helipod/notifications` mounts `POST /api/notifications/webhooks/` when a webhook-capable provider
is configured.
...
`helipod.config.ts` composes. Every deployment gets
`ctx.storage.generateUploadUrl`/`getUrl`/`getMetadata`/`delete`/`store`/`get`, with no entry in
`helipod.config.ts` and no way to opt out.
...
opt-in. Each is composed by name in `helipod.config.ts`, absent from a deployment that doesn't list
