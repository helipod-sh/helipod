...
`@helipod/auth` is a first-party, self-hosted authentication component. Out of the box it gives
...
Compose `auth` in `helipod.config.ts` like any other component:
...
```ts title="app.ts"
import { HelipodClient, webSocketTransport, createAuthClient, anyApi } from "@helipod/client";

const client = new HelipodClient(webSocketTransport(url));
...
- **Persistence.** `localStorageSession()` is the default `SessionStorage`. It reads and writes a
  `helipod.session` JSON blob (the exported constant `SESSION_STORAGE_KEY`), and falls back
...
- **Cross-tab broadcast.** After a winning refresh, the new pair is posted over a
  `BroadcastChannel` (`helipod:auth:pair`) so every other tab adopts it immediately rather than
...
```ts title="helipod.config.ts"
import { defineAuth, consoleEmail } from "@helipod/auth";

export const auth = defineAuth({
  email: {
    provider: consoleEmail(),                // zero-config dev default
    from: "no-reply@example.com",
    appName: "My App",                       // used in email subject lines (default "Helipod app")
...
<TypeTable
  type={{
    ...
    notificationCategory: {
      description: 'The category passed to a composed @helipod/notifications (see Notifications integration below)',
      type: 'string',
      default: '"auth"',
    },
  }}
...
- **`consoleEmail()`** is the zero-config dev default. It delivers nothing: it prints the whole
  ...
  the server's own console, meaning the terminal running `helipod dev`/`serve`.
...
import type { EmailProvider } from "@helipod/auth";
...
```ts
import { defineAuth, consoleEmail, defaultTemplates } from "@helipod/auth";
...
password sign-in uses. Helipod's own sessions always stay DB rows: an external identity is
...
sequenceDiagram
  ...
  participant Engine as Helipod engine
...
mints a Helipod session through the exact same core the OAuth callback uses.

This is an exchange model, not per-request verification. Helipod verifies the token once and
trades it for a real DB-backed session, rather than treating the third-party JWT itself as the
ambient identity re-verified on every request, because:
...
export const auth = defineAuth({
  mfa: {
    encryptionKey: process.env.HELIPOD_AUTH_MFA_KEY!, // a 32-byte key, base64 or hex
    issuer: "My App",                                   // otpauth:// issuer label (default "Helipod")
  },
});
```
...
posture as `HELIPOD_ADMIN_KEY` for the admin and storage surfaces.
...
moment they're shown). The other is an admin or support-side reset your own app builds. Helipod
...
defineAuth({
  mfa: {
    encryptionKeys: [
      { id: "2", key: process.env.HELIPOD_AUTH_MFA_KEY_2! }, // NEW primary, encrypts going forward
      { id: "1", key: process.env.HELIPOD_AUTH_MFA_KEY_1! }, // retired, still decrypts old secrets
    ],
  },
});
```
...
When `@helipod/notifications` is composed alongside `auth`, every A2 email send (OTP,
...
Helipod is WebSocket-first: identity flows over the `SetAuth` message, not request headers. So
...
isolation, front Helipod with your own auth-terminating proxy.
