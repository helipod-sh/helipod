---
title: FAQ
description: Honest answers about what helipod is, what it isn't, and what isn't built yet.
---
...
invalidation (not table-level); optimistic updates; a durable offline outbox; `helipod
...
Multi-node scale is newer, and it lives under a separate license. `helipod serve --fleet` (Tier
...
If you're choosing helipod today, start with single-node self-hosting (SQLite or Postgres). It's
...
See [What is helipod?](/docs/get-started/what-is-helipod) for the full pitch and
...
`helipod migrate` flags a Convex app's use of either as unsupported in its migration report
...
helipod serves plain HTTP. Put a reverse proxy (nginx, Caddy, Traefik) in front for HTTPS. This
...
Within `@helipod/workflow`: a workflow can't start and await another workflow as one of its own
...
MinIO, and its tail items have shipped since: a multi-shard writer (`helipod serve
...
`HELIPOD_OBJECTSTORE_GC_MS`), and an offline reshard tool (`helipod objectstore reshard`).
...
the right choice for `helipod dev`, and for self-hosted single-node production too. It's
...
(or `HELIPOD_DATABASE_URL`), no code changes. It's also the required shared substrate if you're
...
- A single-binary compile (`helipod build`) embeds the engine, your app, and the dashboard in one
  executable, useful for Electron/Tauri-style distribution.
- Native `@helipod/*` imports are canonical, not `convex/*`. helipod is its own product, not a
  Convex account or drop-in replacement. `helipod migrate --from convex` is the on-ramp (see
  [Migrate from Convex](/docs/reference/migrate-from-convex)), not the identity.
- helipod has already grown past Convex's shipped feature set in some areas: durable workflows
...
`ctx.db.replace(id, { ...doc, ...changes })`. `helipod migrate` flags every `.patch(...)` call it
...
| metric | helipod | Convex |
|---|---:|---:|
| reactive propagation p50 (50 subscribers) | **8.6 ms** | 13.4 ms |
| reactive propagation p99 | 13.7 ms | 27.3 ms |

Same order of magnitude, and in this same-substrate test, on par with or ahead of the commercial
Rust reference it's modeled on. See [Performance](/docs/get-started/performance) for the full
scorecard and its caveats. helipod is a clean-room build studied against Convex's open, published
...
- helipod's reactivity comes from one mechanism, in one process: deterministic TypeScript
  functions with recorded read/write sets, intersected at commit time. There's no rules DSL to gate
  access (authorization is ordinary function code, optionally composed via `@helipod/authz`'s row
  policies), and no fleet of services to run. One process is the whole backend.

There's also the deploy axis. helipod deploys anywhere you can run a container, a binary, or a
...
Yes. helipod is licensed FSL-1.1-Apache-2.0 (the Functional Source License, the same license
Convex itself uses). That means free to use, modify, and self-host, including at scale, on your own
infrastructure. The license forbids exactly one thing: offering helipod itself as a competing
...
- **Deploy anywhere.** Your box, your VPS, your cloud, Docker, an air-gapped server. No phone-home,
  ever.
- **Data and code portability.** Plain HTTP, open formats, `helipod migrate` in and `helipod
...
(`@helipod/fleet`, Postgres-backed) and the Cloudflare-native multi-shard router
(`@helipod/runtime-cloudflare-shard`), lives in a separate `ee/` area under a different commercial
...
documented as a feature anywhere else in these docs, and `helipod migrate` flags a Convex app's
...
Every number helipod quotes is hand-transcribed from a runnable benchmark harness under
...
Bun is primary: `helipod dev`/`serve`, and the single-binary compile (`bun build --compile`). But
Node is fully supported for running the engine (npm packages, a Node SQLite adapter). The engine
itself is runtime-agnostic behind its storage/runtime seams. Neither `helipod dev` nor `serve`
cares which one you're running, as long as the seam is satisfied.

## Can I run it on Cloudflare?

Yes, via `@helipod/runtime-cloudflare`, in one of two genuinely different architectures. The
DO-native path is a first-class deploy target: `helipod deploy --target cloudflare` reconciles
...
`wrangler deploy` of the portable `helipod serve` image.
...
flowchart TD
  ...
  Worker -->|"forwards every request/upgrade to one DO by name"| DO["HelipodDurableObject"]
  subgraph internals[" "]
...
that's a separate paid-tier package, `@helipod/runtime-cloudflare-shard`).
...
It runs the exact same portable `helipod serve` image you'd run under Docker anywhere else,
...
- **The same app code runs everywhere.** Your `schema.ts` and `helipod/` functions run unchanged on
  `helipod dev`, `helipod serve` (SQLite or Postgres), a compiled single binary, a multi-node
...
- **Your data moves too, explicitly.** `helipod migrate export`/`helipod migrate import` pulls
  ...
  two hosts or topologies, because every helipod store shares the same logical MVCC-log shape.
...
project by listing it in `helipod.config.ts` (for example, `defineAuth()`, `defineScheduler()`).
...
- New to helipod: [What is helipod?](/docs/get-started/what-is-helipod) →
