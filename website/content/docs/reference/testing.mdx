---
title: Testing
description: Test functions against the real engine with @helipod/test.
---

{/* diataxis: how-to */}

Most test harnesses mock the database and hope the mock behaves like the real thing. `@helipod/test`
...
This page is the complete reference for the harness. It covers every option `createTestHelipod`
takes, every method on the object it returns, how to test reactivity and time-based (scheduler and
cron) code, how to compose components, and the specific harness behaviors worth knowing before you
write assertions.
...
```ts title="helipod/messages.test.ts"
...
`createTestHelipod` is async. Always `await` it, since it boots a real runtime: schema setup,
...
function createTestHelipod(opts: CreateTestOptions): Promise<TestHelipod>;
```
...
<TypeTable
  type={{
    ...
    components: {
      description: "Components to compose, the same ones your app opts into via helipod.config.ts.",
...
A flat map from a module path (`"messages.ts"`) to that module's exports. Two shapes
work:
...
const t = await createTestHelipod({
  modules: import.meta.glob("./helipod/**/*.ts"),
});
```

Keys from `import.meta.glob` come back with leading path segments, like `./helipod/messages.ts`. The
harness normalizes them: it strips `./`/`../` prefixes, a leading `helipod/` root, and the file
...
If your app opts into a component in `helipod.config.ts` (the scheduler, workflow, triggers,
...
closing it in `t.close()`. This is how Helipod's own suite proves read-your-own-writes parity on
...
## `TestHelipod`, the returned object
...
Routes a `Request` through your app's `http.ts` router exactly as the real `helipod dev`/`serve`
...
`http.ts`'s default export (an `HttpRouter`) is resolved once at `createTestHelipod` time; a route
...
The identity reaches function code only through a context provider (for example `@helipod/auth`'s
...
```mermaid
flowchart LR
  Test["t.subscribe(...)"] -->|"loopback"| Client["HelipodClient"]
...
One `HelipodClient` is shared across every `t.subscribe(...)` call on a backend and all of its
...
Deterministic time control for `@helipod/scheduler` jobs and crons, with no real timers or sleeps
involved. Both are driven by the harness's own virtual clock (starting at a fixed epoch millisecond,
mutable only through these two calls). Both are also clean no-ops if no `@helipod/scheduler`
...
- **Both throw if `opts.now` was supplied** to `createTestHelipod`. With a custom clock, the caller
...
const t = await createTestHelipod({ modules: { /* ... */ }, now: () => Date.now() });
...
Every `createTestHelipod()` call is a fully independent backend: its own SQLite `:memory:` database
...
```ts
const a = await createTestHelipod({ modules /* ... */ });
const b = await createTestHelipod({ modules /* ... */ });
...
Any component your app opts into via `helipod.config.ts` (the scheduler, workflow, triggers,
...
would under `helipod dev`/`serve`. File storage (`ctx.storage`, the `_storage` table, the orphan
reaper driver) is composed unconditionally, without needing to be listed. It's always-on in
production too.

## Conformance suite

Helipod's own repository ships a conformance suite: a large body of tests, all written against
`@helipod/test`, whose entire purpose is to pin down and regression-guard the shipped engine's
...
The rule the suite holds itself to: assert real, observed behavior, and where a behavior might
surprise you, document it explicitly (see the next section) rather than silently asserting the wrong
thing to keep a test green.

## Harness behaviors worth knowing

A handful of harness behaviors surface often enough in tests to call out directly. Each one was
pinned down by the conformance suite against the actual shipped engine.
...
`@helipod/auth`'s `ctx.auth`, or a component reading `cctx.identity`). There is no bare
...
`hasMore` is `false` only on the last page. `scanCapped` is `true` when the page's underlying scan
hit `maxScan` before filling `pageSize`. That's a signal that more matching rows may exist beyond
...
`helipod migrate` rewrites your Convex import specifiers automatically, but it does **not**
translate `convex-test` calls to `@helipod/test`. See
...
A few error behaviors, beyond the schema-validation behavior above, that show up often enough in
...
`@helipod/test`'s in-process harness (everything above) is the right tool for function and
...
protocol, Docker boot, `helipod deploy`'s live hot-swap, cross-runtime Bun vs. Node behavior), start
a real `helipod dev`/`serve` as a child process and drive it with `@helipod/client` over a real
WebSocket, or `POST /api/run` over HTTP. This is exactly the pattern Helipod's own end-to-end tests
...
There is no `helipod run` command and no separate `createHelipod` testing API. Use
`@helipod/test` for in-process tests, and `POST /api/run`, the dashboard's function runner, or the
...
- [Schema & tables](/docs/core-concepts/schema-and-tables): `defineSchema`, indexes, and
  `schemaValidation`.
- [Migrate from Convex](/docs/reference/migrate-from-convex): what `helipod migrate` rewrites
