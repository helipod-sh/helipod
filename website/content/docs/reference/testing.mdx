...
```ts title="stackbase/messages.test.ts"
...
const t = await createTestStackbase({
  modules: import.meta.glob("./stackbase/**/*.ts"),
});
```

Keys from `import.meta.glob` come back with leading path segments, like `./stackbase/messages.ts`. The
harness normalizes them: it strips `./`/`../` prefixes, a leading `stackbase/` root, and the file
