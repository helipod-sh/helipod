...
**Streaming reads on Postgres** (added in 0.0.8): paginated and limited reads (`.paginate()`, `.take(n)`) stream from a server-side cursor that stops fetching the moment the query engine has enough, instead of materializing the whole index range. Measured at 100k rows on the paginated shape: **−92% p50 wall-clock** and roughly 1000x fewer rows fetched. On by default, with a `HELIPOD_PG_STREAM=0` kill switch.
