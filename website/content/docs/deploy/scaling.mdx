...
helipod runs as a set of tiers, not one fixed shape. Think of it as a building with three floors.
You write your schema, queries, mutations, and subscriptions once, on the ground floor, and the
same code runs unchanged whether you're on a laptop running `helipod dev`, one `helipod serve`
...
flowchart TB
  subgraph T1["Tier 0/1: one process"]
    A1["helipod serve"] --> D1[("SQLite or Postgres")]
  end
...
`helipod serve` process is one writer: every mutation goes through it, fully serializable, no
...
## Tier 2: the fleet (`@helipod/fleet`)

`helipod serve --fleet` runs a small, symmetric fleet of `helipod serve` processes that share
...
```bash
export HELIPOD_ADMIN_KEY=secret
export HELIPOD_DATABASE_URL=postgres://user:pass@host:5432/db
export HELIPOD_FLEET=1
export HELIPOD_ADVERTISE_URL=http://10.0.0.5:3000
...
- **`--fleet` / `HELIPOD_FLEET`** turns fleet mode on.
- **`--database-url` / `HELIPOD_DATABASE_URL`** must point at Postgres. SQLite has no concept of
  a shared lease across processes, so `--fleet` against SQLite (or no database URL at all) fails
  fast at boot.
- **`--advertise-url` / `HELIPOD_ADVERTISE_URL`** is the URL *other* fleet nodes use to reach this
...
- **The same `HELIPOD_ADMIN_KEY` on every node.** Nodes authenticate to each other's internal
  forwarding endpoint with this key.
- **A unique data directory per node.** A node's local replica lives at `<dir>/fleet-replica.db`
  alongside whatever `--data`/`HELIPOD_DATA_DIR` you gave it. This isn't validated: two nodes
  sharing a directory silently stomp each other's replica file instead of failing fast.
- **`@helipod/fleet` installed.** `serve` loads it via a dynamic `import()` (core helipod has no
...
missing advertise URL, `@helipod/fleet` not installed, or `--fleet` combined with
...
variable, are in [the CLI reference](/docs/core-concepts/cli#helipod-serve).
...
- **A writer that's alive but stuck** (a GC pause, a frozen process, a hung transaction) is a
  ...
  writer's lease carries a time limit, **`HELIPOD_FLEET_LEASE_TTL_MS`** (default `15000`), that it
...
- Lowering `HELIPOD_FLEET_LEASE_TTL_MS` makes a wedged-writer takeover faster at the cost of a
...
helipod recognizes the marker and hands back the original result instead of running the write a
...
running `helipod dev`, a single `helipod serve`, or a fleet. `helipod dev` runs the full shard
...
and if a wedged writer can never corrupt or fork a shard's history. Helipod's design for this,
...
### Shard count: `HELIPOD_FLEET_SHARDS`

The shard count is a **deployment-wide constant**, not per-table or per-app:

- **Default: 8**, whether on `helipod dev`, a single `serve`, or a fleet.
- **Set explicitly with `HELIPOD_FLEET_SHARDS`** (a positive integer) before the deployment's very
  first boot. It's read once, persisted, and every later boot reads the persisted value back.
- **Immutable after first boot.** A `HELIPOD_FLEET_SHARDS` value that disagrees with what's
...
### Changing shard count on a stopped fleet: `helipod fleet reshard`

```bash
helipod fleet reshard --shards 16 --database-url postgres://user:pass@host:5432/db
```

This changes a **stopped** Postgres fleet's shard count N to M. It's Postgres-only (`--database-url`,
or `HELIPOD_DATABASE_URL`, must resolve to a real Postgres URL) and refuses to run against a fleet
...
`HELIPOD_FLEET_SHARDS` (or unset it) on every node before restarting the fleet. The command's own
...
`HELIPOD_GROUP_COMMIT=1` batches concurrent writes on a single shard (or an unsharded fleet, which
...
```bash
export HELIPOD_FLEET_MULTI_WRITER=1
```
...
- **Failover is per-shard.** Losing a writer node stalls only the shards it owned, not the whole
  ...
  `HELIPOD_FLEET_LEASE_TTL_MS` timing as single-writer failover.
...
(`HELIPOD_FLEET_MULTI_WRITER=1`) over one shared Postgres, 4 shards per node, driver routing every
...
## Tier 3: the object-storage substrate (`@helipod/objectstore-substrate`)
...
```bash
HELIPOD_ADMIN_KEY=secret helipod serve --dir helipod \
  --object-store s3://accessKey:secretKey@localhost:9000/my-bucket?region=us-east-1
```

- **`--object-store` / `HELIPOD_OBJECT_STORE`** selects the backend by URL scheme: `s3://` /
...
- **`--shards N` / `HELIPOD_FLEET_SHARDS`** (object-store boots only) sizes a **multi-shard
...
- **`--replica` / `HELIPOD_REPLICA`** boots this node as a **read-only replica** of an
  ...
  `--object-store`.
- **`--writer-url` / `HELIPOD_WRITER_URL`** (replica boots only) makes a replica **forward**
...
- **`HELIPOD_OBJECTSTORE_GC_MS`** sets the writer's background garbage-collection sweep cadence
  (default ~60 seconds), reclaiming segments and snapshots already superseded by a later snapshot.
- **`--wake-url` / `HELIPOD_WAKE_URL`** and **`--backstop-min-ms` / `HELIPOD_BACKSTOP_MIN_MS`**
...
[the CLI reference](/docs/core-concepts/cli#helipod-serve).
...
### Changing shard count: `helipod objectstore reshard`

```bash
helipod objectstore reshard --object-store s3://... --dir helipod --shards 4
```
...
Both `@helipod/fleet` (Tier 2) and `@helipod/objectstore-substrate` (Tier 3) are `ee/`-licensed
packages: source-available under helipod's separate, non-converting commercial license, not the
...
Core helipod has no hard dependency on either `ee/` package: `serve --fleet` and
