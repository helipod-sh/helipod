...
description: Live hot-swap onto a running server with helipod deploy, or compile a self-contained binary with helipod build.
---

{/* diataxis: how-to */}

You've got a `helipod serve` deployment running (see [Self-hosting](/docs/deploy/self-hosting)). Now
...
- `helipod deploy` pushes local `helipod/` changes to an already-running `serve` and hot-swaps them in
  live, with no restart.
- `helipod build` compiles your whole app into one self-contained executable, for when you'd rather
  ship a binary than a Docker image.
- The provisioning targets (`cloudflare`, `docker`, `railway`, `fly`, `aws`), a different
  `helipod deploy` mode entirely. They provision infrastructure, they don't hot-swap.

This page covers all three. If you already know what you're looking for, jump straight there:

- [`helipod deploy`: live hot-swap](#helipod-deploy-live-hot-swap-onto-a-running-server)
- [The six deploy targets](#the-six-deploy-targets)
- [Deploying from CI (GitHub Actions)](#deploying-from-ci-github-actions)
- [`helipod build`: one self-contained binary](#helipod-build-one-self-contained-binary)
...
[the CLI reference](/docs/core-concepts/cli#helipod-deploy).
...
<Callout type="warn" title="Functions must be top-level under helipod/">
...
1. **Transpiles every `.ts` file under `helipod/`** via esbuild's `transform` (not `bundle`). This strips
   TypeScript types but leaves import specifiers completely untouched. A bare import like
   `import { v } from "@helipod/values"` passes through as-is, and it resolves against the target
   server's own `node_modules`, not anything on your machine. Relative imports resolve within the
   pushed file tree itself. This is why a deploy target needs the same `@helipod/*` packages
   installed as your app does (the Docker image already provides this, see
   [Self-hosting](/docs/deploy/self-hosting)). It does **not** refresh `helipod/_generated/`; that's
   `helipod codegen`'s job, run it yourself before deploying.
...
3. **Posts a delta** to `/_admin/deploy`, with `Authorization: Bearer $HELIPOD_ADMIN_KEY`: modules
...
`helipod.config.ts` (see [Configuration](/docs/reference/configuration#the-deploy-block)).
...
- **It never hangs.** `helipod deploy` derives its interactive mode from `stdin.isTTY` and the
...
- **`--check` gates codegen drift.** It exits 1 if `helipod/_generated/` doesn't match a fresh
...
For a provisioning target, swap the deploy line and secrets: `bunx helipod deploy --target
...
`HELIPOD_ADMIN_KEY` is required, exactly like `helipod serve`. The binary fails fast (exit 1) if
...
- Functions and schema: shipping a change means rebuilding and redeploying the binary, not
  `helipod deploy`.
- The composed component set: adding `@helipod/workflow` to `helipod.config.ts` requires a rebuild,
...
- No `helipod init` scaffolder. You bring your own `helipod/` directory. `helipod build` doesn't
  generate one.
...
2. **Generates a static-import entrypoint.** `bun build --compile` only bundles code reached via static
   ...
   `helipod build` writes a throwaway `entry.ts` that statically imports every `helipod/` module by
   absolute path, the schema, `helipod.config.ts` (if present), and, if the dashboard is included,
   ...
   same boot core `helipod serve` uses, just fed statically imported modules instead of a dynamic
   directory scan).
...
helipod build --target linux-arm64 --outfile ./dist/server-linux-arm64
```

```bash tab="darwin-arm64"
helipod build --target darwin-arm64 --outfile ./dist/server-macos
```

```bash tab="darwin-x64"
helipod build --target darwin-x64 --outfile ./dist/server-macos-x64
```

```bash tab="windows-x64"
helipod build --target windows-x64 --outfile ./dist/server-windows.exe
```
...
EXPOSE 3000
VOLUME /data
ENTRYPOINT ["/helipod-server", "--hostname", "0.0.0.0", "--data-dir", "/data"]
```

```bash
helipod build --target linux-x64 --outfile ./dist/helipod-server
docker build -f Dockerfile.binary -t my-app-binary .
docker run -p 3000:3000 -e HELIPOD_ADMIN_KEY=your-strong-secret -v helipod-data:/data my-app-binary
```
...
- [Self-hosting with Docker](/docs/deploy/self-hosting): the `serve` deployment `helipod deploy`
  pushes onto, and the bind-mounted-image alternative to the standalone binary.
- [Postgres](/docs/deploy/postgres): `--database-url`/`HELIPOD_DATABASE_URL`, accepted by both
  `serve` and the compiled binary.
- [Cloudflare](/docs/deploy/cloudflare): `helipod deploy --target cloudflare`, and the general
  `--target`/`--env` provisioning model.
- [Local dev](/docs/deploy/local-dev): `helipod dev`, the hot-reload loop this page's `deploy` and
  `build` both build on.
