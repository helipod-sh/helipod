...
description: helipod serve and docker compose up, the baseline production self-host path.
---
...
This is the baseline way to self-host helipod. A generic `helipod:latest` image runs
`helipod serve`, your app's `helipod/` folder is mounted in, and its SQLite database sits on a
...
`helipod serve` is `helipod dev` minus the file watcher and codegen-write, plus production
hardening.
...
| Admin key | Ephemeral, auto-generated, embedded in the dashboard HTML | **Required** (`HELIPOD_ADMIN_KEY`), never embedded |
| File watching | Watches `helipod/` and hot-reloads | None, restart (or `helipod deploy`) to pick up changes |
...
1. **`HELIPOD_ADMIN_KEY` must be set and non-blank.** There is no default and no fallback. A
...
routes, any component-contributed routes (for example `@helipod/auth`'s OAuth callbacks), and,
...
`HELIPOD_ADMIN_KEY` to, and it's used from then on to authenticate `/_admin` calls.
...
|---|---|---|---|
| `--dir <path>` | | `helipod` | The app directory to load (schema, functions, `helipod.config.ts`, `_generated/`). |
| `--data <path>` | `HELIPOD_DATA_DIR` (sets `<dir>/db.sqlite`) | `./data/db.sqlite` | SQLite database file path. Ignored when `--database-url` selects Postgres. |
| `--ip <address>` | | `0.0.0.0` | Bind address. |
| `--port <n>` | `PORT` | `3000` | Bind port. |
| `--no-dashboard` | `HELIPOD_DASHBOARD=off` | dashboard on | Disable the dashboard SPA entirely (not just hide it, the route isn't served). |
| `--allow-deploy` | `HELIPOD_ALLOW_DEPLOY=1` | off | Enable `POST /_admin/deploy`, the target of `helipod deploy`'s live hot-swap. See [Deploy and build](/docs/deploy/deploy-and-build). |
| `--database-url <url>` | `HELIPOD_DATABASE_URL` | unset (SQLite) | Point at a Postgres database instead of the SQLite file. See [Postgres](/docs/deploy/postgres). |
| `--storage-bucket <name>` | `HELIPOD_STORAGE_BUCKET` | unset (local FS) | Selects the S3-compatible file-storage backend; presence of a bucket is what switches `ctx.storage` from local disk to S3/MinIO/R2. |
| `--storage-endpoint <url>` | `HELIPOD_STORAGE_ENDPOINT` | AWS default | Custom S3 endpoint (MinIO, R2, etc.), only meaningful alongside a bucket. |
| `--web <dir>` | `HELIPOD_WEB_DIR` | unset | Serve a static frontend (`index.html` + assets) at the site root, same origin as the sync WebSocket. |

`HELIPOD_ADMIN_KEY` is required and has no flag equivalent. That's deliberate: a secret belongs in
...
You need a `helipod/` directory with **committed `_generated/`**. `serve` fails fast without it, as
...
(`HELIPOD_ADMIN_KEY: ${HELIPOD_ADMIN_KEY:?set HELIPOD_ADMIN_KEY in a .env file}`). Compose
...
bind-mounts `./helipod` **read-only** into `/app/helipod`, and persists SQLite on the named
`helipod-data` volume at `/data/db.sqlite`. The container's command is exactly
`serve --dir /app/helipod --data /data/db.sqlite`.

```mermaid
flowchart LR
  AppDir["./helipod (read-only)"] -->|"bind mount"| Container["helipod:latest running serve"]
  Container -->|"port 3000"| You["Your browser or client"]
  Container --> Data[("helipod-data volume: db.sqlite")]
```
...
The SQLite database lives on the `helipod-data` Docker volume, not inside the container's
...
`@helipod/*` package links **nested per-package**, for example
`/app/packages/cli/node_modules/@helipod/executor`, never at `/app/node_modules`.

That's fine for code living inside a workspace package. But your bind-mounted `/app/helipod` is not
under any package. Its bare `import "@helipod/values"` (every `schema.ts`) and the
`_generated/server` module's re-export of `@helipod/executor` resolve by walking **up** to
...
`@helipod/*` name straight into `/app/node_modules/`, so a mounted (or baked-in) `helipod/`
...
(The same `RUN` also creates and chowns `/app/.helipod-deploy`, the scratch directory
`helipod deploy`'s push target writes to. It's the same class of bug: `/app`'s directory node
...
The default compose file bind-mounts `helipod/` at run time. That's convenient for local
...
separate `helipod/` directory around, build a small wrapper `Dockerfile` on top of
`helipod:latest` that copies your app in instead of mounting it:

```dockerfile title="Dockerfile"
FROM helipod:latest
COPY ./helipod /app/helipod
# ENTRYPOINT/CMD are inherited from helipod:latest:
#   serve --dir /app/helipod --data /data/db.sqlite
```

```bash
docker build -t myapp:latest .
docker run -p 3000:3000 -e HELIPOD_ADMIN_KEY=... -v helipod-data:/data myapp:latest
```

Because the root-`node_modules` symlink fixup above is part of the base `helipod:latest` image
(baked into the `runner` stage, not something the bind-mount does at run time), a `COPY`ed
`helipod/` resolves its `@helipod/*` imports exactly the same way a mounted one does. No extra
...
`serve` also accepts `--fleet`/`HELIPOD_FLEET`, `--advertise-url`, `--object-store`,
...
`HELIPOD_DATABASE_URL`. No other change to the compose file's shape is required beyond swapping
...
host `helipod serve`; Docker is the baseline, not a requirement. The checklist is always the
same: set `HELIPOD_ADMIN_KEY` as a platform secret (never in a config file), mount a persistent
...
Set the secret with `fly secrets set HELIPOD_ADMIN_KEY=...`, and start the process as
`helipod serve --dir ./helipod --data /app/data/db.sqlite`. Fly terminates TLS at its edge.

</Tab>

</Tabs>

A managed Postgres on either platform works through the same `HELIPOD_DATABASE_URL` as anywhere
else. Both platforms also have an automated push path: `helipod deploy --target railway` and
...
helipod serves plain HTTP. `serve` has no TLS termination built in. Put a reverse proxy (nginx,
Caddy, or Traefik) in front of the container to terminate TLS and forward to `helipod:3000`.
...
- [Postgres](/docs/deploy/postgres): swap the SQLite volume for a Postgres database, still
  single-node.
- [Deploy and build](/docs/deploy/deploy-and-build): `helipod deploy` for live hot-swap onto a
  running `serve` (no restart, `--allow-deploy` opt-in), and `helipod build` for a single
