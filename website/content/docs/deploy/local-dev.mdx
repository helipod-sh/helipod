...
description: helipod dev's hot-reload server and the built-in dashboard.
---

{/* diataxis: how-to */}

Think of `helipod dev` as your app's dev server, except the backend behind it is the real one, not
...
From your app's root (where `helipod/` lives):
...
1. **Loads your project.** It resolves `helipod.config.ts` (component composition), then reads
   `helipod/schema.ts` and every function file under `helipod/`.
2. **Generates `helipod/_generated/`**: the typed `api`/`Doc`/`Id` helpers your functions and client
   import.
...
5. **Starts watching `helipod/`** for changes and hot-reloads your functions on every save (see
...
```
(dashboard SPA not built: run `bun run --filter @helipod/dashboard build`)
```
...
Here's every flag `helipod dev` accepts:
...
| `--dir <path>` | `helipod` | Where your schema and functions live. |
| `--data <path>` | `.helipod/data.db` | SQLite database file path. Ignored when `--database-url` is set. |
...
| `--database-url <url>` | unset | A `postgres://` or `postgresql://` connection string. When set, helipod uses Postgres instead of SQLite; see [Postgres](/docs/deploy/postgres). Also readable from `HELIPOD_DATABASE_URL`, though the flag wins. |
| `--storage-bucket <name>` | unset | Select the S3-compatible file storage backend (bucket name). Unset means local filesystem storage under `<data dir>/storage`. Also readable from `HELIPOD_STORAGE_BUCKET`. |
| `--storage-endpoint <url>` | unset | S3-compatible endpoint (MinIO, R2, and so on), only meaningful alongside `--storage-bucket`. Also readable from `HELIPOD_STORAGE_ENDPOINT`. |
...
Setting `HELIPOD_STORAGE_ENDPOINT`/`REGION`/`PUBLIC_URL` without a bucket is a misconfiguration,
and `helipod dev` refuses to boot from it. Otherwise it would silently write uploads to local disk
when you meant to configure S3. Set the bucket too, or drop the other settings.

See [File storage](/docs/core-concepts/file-storage) for the full byte-storage story.
...
- If `HELIPOD_ADMIN_KEY` is set in your environment, `helipod dev` uses it as is.
- If it's unset, `helipod dev` generates a fresh ephemeral key on every run and prints it to
  stdout. Nothing persists it. Restart the server and you get a new one.
- If `HELIPOD_ADMIN_KEY` is set but blank or whitespace, `helipod dev` treats it as unset (a
...
A persistent `HELIPOD_ADMIN_KEY` is never embedded in the HTML, loopback or not.

</Callout>

## Hot reload

`helipod dev` watches `helipod/` (recursively) via a filesystem watch, ignoring anything under
`helipod/_generated/` so codegen's own writes don't re-trigger themselves. On every change, after a
short debounce, this is the loop:

```mermaid
flowchart LR
  Edit["You save a file in helipod/"] --> Debounce["Watcher debounces"]
...
2. Re-writes `helipod/_generated/` with the freshly generated types.
3. Calls `runtime.setModules(...)` with the new function map. This replaces the module map
   wholesale, so `helipod dev` re-applies the always-on `_storage:*` built-in functions on every
   reload. They aren't part of your `helipod/` source, so they'd otherwise vanish on the first save.
...
```bash
helipod codegen
helipod codegen --dir helipod   # if your functions aren't in ./helipod
```

This regenerates `helipod/_generated/` and exits: no server, no watch loop. `helipod dev` does this
automatically on every change. You need the explicit command before
[`helipod serve`](/docs/deploy/self-hosting), which never runs codegen itself (it fails fast at
boot if `_generated/` is missing).

## Call a function without the dashboard

There's no `helipod run` command. Either use the dashboard's [function runner](#function-runner),
...
Open the URL `helipod dev` prints: `http://localhost:<port>/_dashboard`. In dev mode, on a loopback
...
That's the right tradeoff for local development, and it's why `helipod dev` hands you the key with
zero friction on a loopback bind. But never expose an unprotected admin key over the network.

[`helipod serve`](/docs/deploy/self-hosting) serves the same dashboard build in production, but
key-less by default (no key embedded in the HTML at all: it prompts you), and requires you to set a
real `HELIPOD_ADMIN_KEY` before it will even start.
...
- [Self-hosting with Docker](/docs/deploy/self-hosting): `helipod serve`, the production
  counterpart of `helipod dev` (no codegen, required admin key, binds `0.0.0.0`, dashboard served
  key-less).
- [Deploying without a restart](/docs/deploy/deploy-and-build): `helipod deploy`, for pushing a
  schema and function change to an already-running `serve` without hot-reload's dev-only watch loop.
- [Vite plugin](/docs/deploy/vite-plugin): a Vite app can run `helipod dev` for you as part of
  its own dev server.
