---
title: Contributing to helipod
description: "An orientation to how helipod is built: the architecture, the codebase, and how to extend it."
icon: Compass
---

{/* diataxis: explanation */}

This section is for a different reader than the rest of the docs. It's for people who want to work *on* helipod itself: reading the engine source, fixing a bug, adding a storage adapter, or writing a new component. If you just want to use helipod in your app, go to [Get Started](/docs/get-started/what-is-helipod) or [Core Concepts](/docs/core-concepts/schema-and-tables) instead.

## helipod in one breath

helipod is an open-source, self-hostable, reactive backend. You write ordinary TypeScript functions: queries, mutations, and actions. They run on the server inside a transaction.
...
- The familiar *authoring style* (how you write a query or a schema) is not a promise to be a drop-in replacement for anything. helipod's canonical imports are its own (`@helipod/*`), and it has already grown capabilities its prior art doesn't have: durable workflows with saga/compensation, a Postgres adapter, a single-binary build. Compatibility with an existing project shows up as a migration on-ramp (`helipod migrate`), not the product's identity.
...
- **[Extending](/docs/contributing/extending/custom-component)**: the seams you can build against without touching the engine core.
  - [A custom component](/docs/contributing/extending/custom-component): like `@helipod/auth` or `@helipod/scheduler`.
...
- The reactive core.
- Single-node/single-binary production tooling: `helipod serve`, Docker self-host, live hot-deploy, a compiled standalone binary.
