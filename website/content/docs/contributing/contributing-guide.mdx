...
description: "How helipod changes get proposed, reviewed, tested, and landed."
icon: GitPullRequest
---

{/* diataxis: how-to */}

This page is the process guide: how an idea becomes a shipped change in helipod. If you're
...
The short version: helipod treats "how we build" as seriously as "what we build." Every
...
Helipod isn't built feature-by-feature in a random order. It's built in **slices**, each one a
...
`helipod dev` starts up aren't polish on top of the "real" product. They are the product. A
...
*uses* helipod. `docs/dev/` and `docs/superpowers/` are internal engineering material:
architecture notes, specs, plans. If you're documenting a change, write it into the audience it
belongs to, not both at once.

### Canonical imports are `@helipod/*`

Even though helipod's runtime is Convex-shaped and a `helipod migrate` on-ramp exists for
Convex users, `@helipod/*` is the one documented, canonical import surface. `convex/*`
...
real `helipod dev` or `helipod serve` server**, not just an isolated mechanism test. These
...
Beyond a normal code review, helipod uses staged reviews at each pipeline step: a plan review,
...
Helipod's core is licensed under **FSL-1.1-Apache-2.0** (free to use, modify, and self-host,
including commercially, but you may not offer helipod itself as a competing hosted service;
...
- [Extending helipod](/docs/contributing/extending/custom-component): building a new component,
