...
description: "Why helipod uses FSL-1.1-Apache-2.0, what's free forever, and how the paid ee/ tier works."
icon: Scale
---

{/* diataxis: explanation */}

If you're evaluating helipod for a real project, the first question is usually: "what am I actually allowed to do with this, and will that change later?" This page answers both, plainly.

The short version: everything you need to build and run a real application is free forever, including self-hosting at scale on your own infrastructure. The one thing you can't do is take helipod and resell it as your own competing hosted service. That's it.

## The license in plain language

Helipod ships under the **Functional Source License (FSL-1.1-Apache-2.0)**. The full text lives in the repository's `LICENSE` file. Here's what it means without the legalese.

You can run helipod in your product, your company, or your side project, at no cost and with no approval needed. You can deploy it on your own server, your own cloud account, or an air-gapped box with no internet access at all. You can fork it, patch it, and read every line of it, since it's all open source.

<Callout type="info" title="The one restriction">

You may not take helipod and offer it back out as a competing hosted product or service, something like "Helipod Cloud, run by someone who isn't us." That's the only thing FSL forbids.
...
flowchart LR
    ...
    C["You resell helipod\nas a competing hosted service"] -->|forbidden| D["Not permitted"]
    E["2 years after release"] --> F["Auto-converts to Apache 2.0\nrestriction lifts entirely"]
```

## Why FSL, and not plain MIT or Apache

"Free" doesn't have to mean "permissive." Those are two different things, and helipod picks free to use but not free to strip-mine.

The engine is the product here. Take a project like Supabase: its real moat is Postgres, something it doesn't own. Helipod is different, its entire value is the reactive engine itself. A permissive license (MIT or Apache) would let any well-funded cloud provider take that engine, host it, and compete directly with helipod's own future business, without contributing anything back. FSL closes that gap and changes nothing for ordinary users and self-hosters.
...
Helipod avoids that trap by setting the protective license once, from the very first commit, before there's a community to betray. Any future paid feature is then just a normal new release under terms that were already public, not a reversal.
...
- Your data and your code stay portable: plain HTTP APIs, open formats, `helipod migrate` to bring data in, and nothing stopping you from taking it back out. You're never locked in.

None of this requires a license key today, and none of it is expected to require one later. See [What is helipod?](/docs/get-started/what-is-helipod) for what the full engine actually includes, and [Self-hosting](/docs/deploy/self-hosting) for how deploy-anywhere works in practice.

## The business model: free now, gate scale later

Helipod isn't a charity. It needs a way to fund ongoing development, so the plan is sequenced on purpose: win adoption first with a genuinely complete free product, and only later, after real popularity rather than a fixed date, introduce a paid unlock for one specific thing. That thing is running helipod's distributed, multi-node scale-out tier, plus a bundle of enterprise features like SSO and audit logging.

This is often called the n8n / GitLab-EE model: sell license keys, not compute. There's no managed cloud hosting business planned. You still deploy the paid tier on your own infrastructure, the same as everything else. The key unlocks a capability, not a place to run it: the rough shape is a signed key, verified offline at startup with no network call back to helipod, so a self-hosted or air-gapped deployment is never at the mercy of a phone-home check to keep running. (The mechanics are designed but not built. More on that in the caveats below.)
...
The `ee/` directory in the repository (short for "enterprise edition," the same convention GitLab popularized) is where paid-tier code lives. It's not a placeholder: `ee/packages/` already holds three shipped packages, **fleet** (the distributed multi-node Tier 2 coordination layer), **objectstore-substrate** (the object-storage-backed storage/compute-separation substrate), and **runtime-cloudflare-shard** (the sharded Cloudflare Durable Object host). The paid *gate* itself isn't built or active yet, though: nothing in `ee/` currently requires a license key to use.
...
- [Architecture: system design](/docs/contributing/architecture/system-design): how the engine that this license protects is put together.
