...
Everything in Helipod, queries, mutations, reactivity, the dashboard's data browser, ultimately
...
up. No prior Helipod knowledge assumed.
...
Helipod's storage layer does something different: it **never overwrites**. Every write, insert,
...
```mermaid
flowchart TD
    Engine["Transactor / query engine<br/>(imports only the DocStore interface)"] --> Iface["DocStore interface<br/>@helipod/docstore"]
...
Helipod ships two database backends today, plus a Cloudflare-native adapter, and the two
