...
`ctx.db.query("messages", "by_author").eq("author", "alice").order("desc").paginate(...)`.
...
- The **range calls** on the builder (`.eq(field, value)`, and one
  `.gt`/`.gte`/`.lt`/`.lte` bound per direction on the next indexed field)
  get compiled into a **scan interval**: a contiguous slice of the index the
  storage layer can jump straight to. Rows outside that slice are never even
  read.
- Anything else (a `.where(op, field, value)` call, including on fields the
  index doesn't cover) becomes a **post-filter**: a check run on each row
  *after* it's already been read out of the scan interval.
...
This lives in the `@helipod/index-key-codec` package
...
`buildIndexInterval`) is to fold the builder's range calls into a single
`[start, end)` interval. Concrete examples first, for a `messages` table with
an index `by_conversation` on `(conversationId, _creationTime, _id)`:
...
The rule that produces those intervals is a walk down the index's field list,
in order:

1. Consume leading **equality** constraints (`.eq`) one field at a time. Each
   one fixes that position in the key, narrowing the range.
...
3. Stop. Fields after that aren't used to narrow the scan at all. If your
   query still needs to constrain them, that part becomes a `.where(...)`
   post-filter instead.
...
- **Update**, and the key is **byte-identical** to before: skip the
  tombstone, but still write a fresh entry at that same key.
...
That last case deserves spelling out, because it's easy to misread as "no
index write at all." Indexes are versioned the same way documents are (see
[Storage & the MVCC log](/docs/contributing/architecture/storage)), so every
new document revision needs an index entry stamped at the new commit
timestamp regardless: `computeIndexUpdates` always emits a fresh
`NonClustered` entry pointing at the document whenever the new document
exists. What the byte-identical check saves is the *tombstone*: when the key
hasn't moved, there's no delete-then-recreate churn at the old key. Update a
message's `text` field with the index on `author`, and the author-index key
stays put; the write is one superseding entry at the same key, not a
tombstone plus an add.
...
Anything that didn't make it into the scan interval (a `.where(op, field,
value)` call, including comparisons like `neq` an index range can't express)
is represented as a small expression tree (`FilterExpr` in `filter.ts`):
comparisons, `and`/`or`/`not`, and dotted field paths like `"author.name"`.
It's evaluated per document, after the row comes back from the scan.
...
(`compareValues`, from `@helipod/values`), never JavaScript's native
...
- Every index secretly ends in `(_creationTime, _id)` so every key is unique
  and totally ordered.
- A query's `.eq`/`.gt`/`.gte`/`.lt`/`.lte` range calls become one
  `[start, end)` scan interval; `.where(...)` becomes a post-filter applied
  per row.
...
- Writes keep indexes in sync, skipping the tombstone (but never the fresh
  entry) when a document's key in that index didn't actually move.
