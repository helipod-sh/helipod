...
A **UDF** ("user-defined function") is the query, mutation, action, or `httpAction` you write in TypeScript, the code that lives in your `helipod/` folder. The executor is the pipeline that takes a request naming one of those functions and turns it into a running handler that reaches the database only through a narrow, controlled doorway, then hands back a result plus a record of exactly what it read and wrote.
...
Two different validation checks happen around every call, both using the same validator system (`@helipod/values`, the same `v.string()`/`v.object()`/etc. helpers app code uses in `schema.ts`):
...
- **Query.** `ctx.db` is a read-only `GuestDatabaseReader` (`get`, `query(...).collect()`/`.paginate()`), plus `ctx.random()`, `ctx.now()`, and a read-only facade for each composed component (`ctx.auth`, `ctx.scheduler`, and so on, whatever the deployment has composed via `helipod.config.ts`).
