...
Think of a file the way helipod thinks of any other row: something you can reference, query, and
react to. The bytes themselves just happen to live somewhere else.

File storage is always on. There's nothing to opt into in `helipod.config.ts`. The moment you run
`helipod dev`, every project has a reserved `_storage` system table and a `ctx.storage` facade
...
2. The **byte transfer** happens outside any transaction, and finishes by confirming, which flips
   the row to `ready`. It takes one of two shapes depending on the backend: **proxied** (bytes go
   through the engine's own HTTP endpoint) or **presigned** (bytes go straight to the bucket). Both
   are covered under [`generateUploadUrl`'s two return
   shapes](#generateuploadurls-two-return-shapes) below.
...
this resolves to a CDN or bucket public URL (when `HELIPOD_STORAGE_PUBLIC_URL` is configured). On
...
<Tabs items={['@helipod/blobstore-fs', '@helipod/blobstore-s3', '@helipod/blobstore-r2']}>

<Tab value="@helipod/blobstore-fs">
...
<Tab value="@helipod/blobstore-s3">
...
URL (public files with `HELIPOD_STORAGE_PUBLIC_URL` set).

</Tab>

<Tab value="@helipod/blobstore-r2">
...
`helipod dev`/`serve`'s env and flag selection below.

</Tab>

</Tabs>

For `helipod dev`/`serve`, the byte backend is picked automatically at boot based on whether an
...
| Flag | Env var | Notes |
|---|---|---|
| `--storage-bucket` | `HELIPOD_STORAGE_BUCKET` | The single switch: set means the S3 backend, unset means the filesystem default. |
| `--storage-endpoint` | `HELIPOD_STORAGE_ENDPOINT` | S3-compatible endpoint URL (MinIO, R2, a non-default AWS region endpoint). Omit for plain AWS S3 with a standard region. |
| (none) | `HELIPOD_STORAGE_REGION` | Bucket region. No CLI flag, env-only. |
| (none) | `HELIPOD_STORAGE_PUBLIC_URL` | Base URL for `"public"` files (e.g. a CDN domain in front of the bucket). Omit to let `getUrl` fall back to signed bucket URLs for private files, and the bucket's own presigned GET for public ones without a CDN. No CLI flag, env-only. |
...
`--database-url`/`HELIPOD_DATABASE_URL` (see [Self-hosting](/docs/deploy/self-hosting)).

The bucket is the *only* switch between the two backends. If `HELIPOD_STORAGE_ENDPOINT`,
`_REGION`, or `_PUBLIC_URL` are set but `HELIPOD_STORAGE_BUCKET` isn't, the server refuses to
...
On the filesystem backend, `helipod dev`/`serve` also fails fast at boot if the storage
