...
A helipod schema is a single TypeScript file. It lists every table in your database and the
shape of the documents each one holds. Your editor and the compiler understand it, and helipod
...
Your `helipod/schema.ts` file default-exports a `defineSchema` call. It maps table names to
...
helipod assigns both on insert. Every table also gets a built-in `by_creation` index (creation
...
`helipod deploy` reason about schema compatibility purely from JSON (see
...
At Tier 0/1 (the default: a single `helipod dev`/`serve` process), `.shardKey` is metadata only.
...
Some tables are built into helipod itself, rather than declared in your `schema.ts`. The current
...
In practice you won't hand-decode ids yourself. `helipod codegen` emits a typed `mintId` in
...
Your schema changes as your app does. helipod's live deploy (`helipod deploy`) enforces an
...
Underneath `schema.ts`, helipod's storage is **schemaless**. On both the SQLite and Postgres
...
- [Configuration reference](/docs/reference/configuration): the same `v.*` catalog in quick
  lookup form, alongside `helipod.config.ts` and environment variables.
