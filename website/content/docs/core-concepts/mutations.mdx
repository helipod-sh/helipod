...
A mutation is a function that writes data. It's the only place in helipod allowed to write:
queries can only read, and actions don't get a `ctx.db` at all.

Every write in a helipod app happens inside a mutation, no matter what triggered it: a client call,
...
generated locally) via the typed `mintId` that `helipod codegen` (and `helipod dev`'s
...
When a project composes a component in `helipod.config.ts`, its facade is attached to `ctx`
