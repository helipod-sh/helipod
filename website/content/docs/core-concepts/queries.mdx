...
well you understand queries is how well you understand why a helipod app stays live.
...
The index name is the second argument to `.query()` itself, and the range methods
(`.eq`/`.gt`/`.gte`/`.lt`/`.lte`) chain directly off the builder:
...
`.paginate()` takes `cursor` and `pageSize` as plain fields, not bundled into an options object.
The result reports `hasMore` (`false` on the last page) and `nextCursor` for the following page.
