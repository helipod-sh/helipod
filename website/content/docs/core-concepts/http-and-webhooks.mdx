...
[Reactivity](/docs/core-concepts/reactivity)). This is the shape every webhook in helipod takes:
...
A conventional `helipod/http.ts` default-exports an `httpRouter()` populated with `route()` calls. This
...
<Callout type="warn" title="No built-in verification">
  ...
  through to the handler's context as its identity. helipod does not decode, validate, or look up
...
`helipod dev`'s watch loop re-resolves `http.ts` on every save, exactly like it reloads your
...
An opt-in composed component (declared via `helipod.config.ts`) can register its own reserved
