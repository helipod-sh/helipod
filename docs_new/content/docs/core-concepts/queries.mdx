...
the same result. Concretely, a query's handler can't call `fetch`, `Date.now()`, `Math.random()`,
...
Not every query gets the same kind of reactive update. The engine classifies each query run by how
"clean" a passthrough it is, and that classification decides whether a later write can be
...
effect on that subscription gets computed and sent. A query classified as a full re-run is exactly
as correct as one classified as diffable, just not free.

Writing queries that stay in the diffable shapes above (a single index-range `.collect()` or
`.paginate()`, returned as-is) is how you keep your subscriptions cheap under load, the same way
narrowing the index range in the first place is how you keep them precise.
