...
Your `convex/schema.ts` file default-exports a `defineSchema` call. It maps table names to
...
<TypeTable
  type={{
    ...
    "v.float64()": {
      description: "Alias of v.number().",
      type: "number",
    },
    "v.int64()": {
      description: "64-bit integer. Alias: v.bigint().",
      type: "bigint",
    },
    "v.bigint()": {
    ...
    "v.literal(value)": {
      description: "Exact match on one specific string, number, bigint, or boolean value.",
    ...
    "v.record(keys, values)": {
      description:
        "An object with an arbitrary, unbounded key set. keys validates each key, values each value.",
      type: "Record<string, Infer<values>>",
    },
    "v.union(...members)": {
      description:
        "Valid if it matches any member. Members are tried in order and the first match wins.",
...
**Rejected (destructive, the deploy is refused, and the running server stays on its current
schema):**
...
- [Mutations](/docs/core-concepts/mutations): the only place `ctx.db.insert`/`replace`/`patch`
