...
flowchart LR
  V["Validate"] --> A["Apply"]
  A --> AL["Allocate"]
  AL -->|"success"| F["Fan out to subscriptions"]
```
...
You never set these directly in the `value` you pass to `insert`/`replace`. Pass them anyway and
they're silently stripped (an ordinary insert) or rejected as a system field. The one exception is
a client-supplied `_id`, covered below.
...
`shardBy` can be a field name (a string naming one of the mutation's `args`, resolved directly
from the call's arguments, as above) or a function `(args) => value` for a computed shard key.
Either way, the resolved value is hashed to a shard **before** the transaction opens, and codegen
...
runs the same virtual-shard machinery), with instructive rejections if you get it wrong. See
**Shard ownership rules** under Going deeper below for the exact list.

If your table has no `.shardKey()` at all, none of this applies. Every mutation and read is
exactly as unsharded as before this feature existed.
...
contains your **app's own tables**. Component and system tables are excluded by construction, so
