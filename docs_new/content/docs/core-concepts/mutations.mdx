...
have no `ctx.db` at all. Every write in a stackbase app, no matter how it was triggered — a client
call, a scheduled job, a webhook, a trigger — happens inside a mutation.
...
- **`args`** is a validator (a record of `v.*` fields). Declaring it is opt-in but strongly
  ...
  with a typed `ArgumentValidationError` (code `ARGUMENT_VALIDATION`, HTTP 400) **before the handler
  ever runs**. A mutation with no `args` accepts anything, unchanged from a function with no
  validator at all.
...
scheduler/cron driver calling a job's target, boot steps — so there is no path that reaches a
handler with unvalidated arguments.
...
```
ArgumentValidationError: arguments to "messages:send" do not match validator: body: expected string, got undefined
```
...
`query(table, index).collect()`, `.paginate()` — see [Queries](/docs/core-concepts/queries)), plus
three write methods:
...
- **`ctx.db.insert(table, value)`** inserts a new document and returns its `_id`. `_id` and
  `_creationTime` are assigned automatically (see below) — never pass them in `value` yourself
  (unless you're deliberately supplying a client-minted `_id`, covered later on this page).
- **`ctx.db.replace(id, value)`** replaces a document's fields entirely, given its `_id`. Throws
  `DocumentNotFoundError` if the document doesn't exist.
- **`ctx.db.delete(id)`** removes a document by its `_id`. A delete of an already-missing id is a
  no-op, not an error.

**There is no `ctx.db.patch()`.** Only `insert`, `replace`, and `delete` exist — `replace` takes
the *whole* document, not a partial update. To change one field, read the document first and write
back every field, changed or not:
...
in-memory buffer and invisible to everyone else (`ctx.db.get` inside the same handler *does* see
your own staged writes — read-your-own-writes) until the handler returns successfully, and then
all of it commits together — or, if the handler throws, none of it does. There is no partial
write: a mutation that inserts three rows and then throws leaves zero rows behind.
...
### The 3-phase commit pipeline

Under the hood, committing a mutation's staged writes is a 3-phase pipeline run under the shard's
single-writer mutex:
...
2. **Apply** — every staged insert/replace/delete is appended to the document log, chaining each
   row's `prev_ts` to its latest *committed* revision (the single-writer lock makes this race-free).
3. **Allocate** — the commit timestamp is allocated inside the store's own atomicity domain and
   stamped onto every row atomically, so there is no window where a timestamp is allocated but not
   yet landed.

Only after all three phases succeed does the engine compute the write set and fan it out to
subscriptions (see [How it works](/docs/get-started/how-it-works) and
[Reactivity](/docs/core-concepts/reactivity)).

### OCC conflicts replay your handler, deterministically

When phase 1 detects a conflict, the engine doesn't fail the mutation — it **replays the handler**:
runs it again from scratch, against a fresh snapshot, up to 8 attempts by default. This is
invisible to the caller in the common case; they see one mutation call that eventually committed
(or, past the retry budget, an `OccConflictError` that propagates).

Replay only works because mutations are **deterministic** — see the next section. If replaying the
same handler against the same (now newer) data could produce a *different* sequence of writes than
the first attempt, OCC retry would be unsound. This is why the purity rule below isn't a style
...
arguments and the same data, to do the exact same writes every time it runs — both for OCC replay
above, and because a mutation is itself deterministic-replayable machinery the rest of the system
leans on.
...
- **`_id`** — a unique id, minted fresh on insert (or, for `replace`, unchanged from the existing
  document — `_id` can never be reassigned by a replace).
...
You never set these directly in the `value` you pass to `insert`/`replace` — pass them and they're
silently stripped (an ordinary insert) or rejected as a system field. The one exception is a
client-supplied `_id`, covered below.

**Caveat: same-transaction inserts share one `_creationTime`.** A `_creationTime` comes from the
transaction's single `snapshotTs`, not a per-row clock read — so multiple documents inserted by
*the same mutation call* all get the identical `_creationTime`. If you insert several rows in one
handler and later need to distinguish their relative order, `_creationTime` alone won't do it (the
default `by_creation` index tiebreaks equal timestamps on the row's random `_id`, which is not a
meaningful order). Add an explicit ordinal/sequence field yourself if you need a deterministic
order among documents created in the same transaction:

```ts title="convex/steps.ts"
...
document in the table to one of the deployment's shards, keyed by the value of that field — the
mechanism Tier-2 (multi-node) scale-out is built on. A table with no `.shardKey()` is **unsharded**:
every document lives on the single **default ring**, which is also where every mutation runs
unless it opts into sharding.
...
`shardBy` can be a field name (a string naming one of the mutation's `args`, resolved directly from
the call's arguments — as above) or a function `(args) => value` for a computed shard key. Either
way, the resolved value is hashed to a shard **before** the transaction opens, and codegen
cross-checks a string `shardBy` against the table's actual `.shardKey()` at build time, so a
mismatch is caught before it ever reaches the engine's runtime guards.

The engine enforces shard ownership at every tier — a single-node deployment included, since it
runs the same virtual-shard machinery — with instructive rejections if you get it wrong:
...
If your table has no `.shardKey()` at all, none of this applies — every mutation and read is exactly
as unsharded as before this feature existed.

## Client-supplied ids

By default, `ctx.db.insert` mints a random `_id`. A mutation can instead accept a **client-minted**
id and pass it straight through as `_id` in the insert value — the engine validates and accepts it
in place of minting its own:
...
This is the primary pattern for an **offline create-then-reference chain**: mint the id before
either mutation is even sent, hand it to both, and both can enqueue into the durable
[offline outbox](/docs/client/offline-sync) without either waiting on the other to actually commit.
On drain, the create runs first (the outbox is FIFO), lands under the minted id, and by the time
the second mutation runs, the reference resolves against a real row.
...
### v1 restriction: unsharded tables, default ring only
...
existence lookup even runs. The reason is concurrency safety, not an arbitrary limitation: the
existence check is a snapshot read scoped to *this transaction's own shard*. On a sharded
deployment, each shard is an independently-mutexed OCC domain with its own snapshot and recent-
commits ring — so two concurrent inserts of the *same* client-supplied id on *different* shards
would each see "not found" locally and both commit, producing a silent duplicate identity. Pinning
the id path to one ring (the default one) makes the existence check globally sound for that table.
...
`mintId`'s type parameter accepts any of your schema's table names, but the emitted map only
contains your **app's own tables** — component/system tables are excluded by construction, so
`mintId("_storage")` compiles but throws at runtime ("unknown table ... regenerate `_generated/`").

### Purity rule for optimistic updates

If you're pairing a client-supplied id with an [optimistic
update](/docs/client/optimistic-updates), mint it **outside** the updater — at args-construction
time, once — and have the updater read the id **from args**, never mint one itself. Minting
consults randomness; an updater must stay pure and safely re-runnable on every ingest, exactly like
`store.placeholderId()`/`store.now()`:
...
When a project composes a component in `stackbase.config.ts`, its facade is attached to `ctx` under
that component's name, and — for components that opt into writing — is usable from inside a
mutation's own transaction, so its writes commit and roll back with the rest of the handler:

```ts title="convex/reminders.ts"
