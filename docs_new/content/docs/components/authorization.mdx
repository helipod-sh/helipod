...
`@stackbase/authz` is how you express all three without hand-rolling permission checks inside every
query and mutation.
...
`permissions`, a documentation-only vocabulary hint (`{ resource: string[] }`). Nothing reads
`permissions` at runtime today. The enforced source of truth is what a role's entries actually grant.
...
A check (`can`) is then a lookup against at most four **candidate keys** for the permission being
asked about: `res:act`, `res:*`, `*:act`, `*:*`. Each is a single indexed point-read via
`byLookup`, not a graph walk:
...
(`ctx.db.get`, `.query(...).collect()`, paginated scans, and relation lookups a policy resolves
through), and a `write` policy gates whether an insert, replace, or delete is allowed:
...
collects parents where **all** related rows match, vacuously true for a parent with zero related
rows, so "at least one, and all match" needs
`{ AND: [{ sharedWith: { some: {} } }, { sharedWith: { every: leaf } }] }`.
`is`/`isNot` work the same way over a `v.id` field with no extra schema declaration needed. Leaves
can themselves contain relation clauses, chaining through multiple tables, for example a document
shared with a *team* the caller belongs to:
