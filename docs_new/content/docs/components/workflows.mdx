...
maybe with a sleep or a wait for an external signal in between. `@stackbase/workflow` runs that
whole sequence durably.

A workflow survives a process restart because it isn't held in memory. Each step's outcome is
journaled to a durable table, and stackbase **replays the whole handler from the top** every time
...
- **`runAction` is at-most-once**: it dispatches a `kind: "action"` scheduler job, and actions
  carry the scheduler's own at-most-once contract. A crash mid-flight dead-letters rather than
...
stateDiagram-v2
    [*] --> running
    running --> compensating: step fails / cancel-with-compensation
    compensating --> failed
