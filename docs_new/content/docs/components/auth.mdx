...
doesn't leak whether the account exists. A successful sign-in resets the counter and, if the
stored hash predates a hashing-cost upgrade, transparently rehashes it in the same write.
...
bounds total guesses across a `signIn → wrong-guesses → signIn → …` loop.
