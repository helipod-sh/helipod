...
The query engine takes a declarative query — the kind of thing you write as
`db.query("messages").withIndex("by_author", (q) => q.eq("author", "alice")).order("desc").paginate(...)`
— and does two jobs with it at once: it turns the query into actual storage
scans, **and** it records the exact key ranges those scans depended on. That
second part is what lets [reactivity](/docs/contributing/architecture/reactivity)
work precisely: when a later write lands inside a range you depended on, your
subscription re-runs. When it doesn't, it's left alone.

Everything in this page is in service of that read-set precision. Get it
right, and a chat app with ten rows and one with ten billion rows run the
exact same code path, and neither one re-runs subscriptions it doesn't need
to.
...
just arithmetic on these byte ranges. There's a strict rule the engine follows
because of this: **no part of the query engine ever compares two decoded
values with JavaScript's native `<`/`>`.** Native comparison disagrees with
the codec on edge cases like `-0` vs `+0`, `NaN`, and bigint-vs-number — so
every ordering decision (sort, cursor position, range membership) goes through
the codec's own `compareKeyBytes`/`compareValues` instead.

## Every index has a hidden tiebreaker

An `IndexSpec` (`packages/query-engine/src/index-manager.ts`) is nothing more
than a table, a name, and an ordered list of field paths — e.g.
...
"[start, end)" number line pictured above — `start` is always inclusive,
`end` is always exclusive, and `end: null` means "no upper bound, scan to the
end of the index."

There's no full search or vector-index planner shipped today — this page
only covers the ordered index-range/table-scan path that ships. See
...
- **`collect(query, ...)`** — scan the whole computed interval and return
  every matching row.
...
This is the crucial subtlety, and it's worth being explicit about because
it's easy to get backwards: **the read set records the interval that was
scanned, not the rows that survived the filter.**
...
- If the scan stopped early — a page filled up, or a scan cap kicked in —
  only the span actually consumed is recorded: from the interval's start up
  to (and including) the last key the scan looked at.
...
worth of keyspace — because a write that changes row #501 (rejected by the
filter, never returned) could flip it to pass the filter on the next run, and
the query needs to know to re-run and check. Recording only the 100 surviving
...
that field — a lot of unnecessary write amplification and unnecessary
reactive churn.
...
(the same `author`, the same `_creationTime` down to the millisecond — ties
happen). Two consequences fall out of this for free:
