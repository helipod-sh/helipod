...
Every backend Stackbase ships today, SQLite and Postgres, talks to the engine through the exact same doorway. Stackbase's engine (the transactor, the query engine, reactivity) never talks to a database directly. It talks to one narrow TypeScript interface called `DocStore`. Whatever sits behind that interface (SQLite, Postgres, a Durable Object's embedded SQLite, or something you write yourself) is invisible to everything above it.
...
- **A Postgres sequence** (`nextval('stackbase_ts')`), drawn from inside the same commit transaction as the row inserts. This is the async equivalent of SQLite's `MAX(ts) + 1`, using Postgres's own atomic counter instead of a table scan.
