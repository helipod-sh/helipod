...
- Write a custom email/SMS/push provider for `@stackbase/notifications`
- Add a new OAuth provider to `@stackbase/auth`
- Write a custom `BlobStore` for file storage

## Notifications: one interface per channel

`@stackbase/notifications` groups delivery into **channels** — `email`, `sms`, `push`, and
...
Each channel's provider is a plain object shape — no base class, no vendor SDK type leaking into
the signature:
...
- Throwing `new NotificationSendError(message, { retryable: false })` marks the failure as
  **permanent** (for example, a 4xx "bad recipient" response) — the driver dead-letters the message
  immediately instead of wasting retries on something that will never succeed.

```ts
// permanent failure — e.g. the vendor returned a 4xx "bad recipient" response
...
`verify` is a security boundary, not a nicety: the shipped `resendEmail` adapter verifies a Svix
HMAC signature with a constant-time comparison and a 5-minute timestamp-skew check before it will
let a webhook event anywhere near a write. Copy that shape rather than skipping verification.
...
```ts
import type { EmailProvider } from "@stackbase/notifications";
...
import { defineNotifications, twilioSms } from "@stackbase/notifications";
...
to add a vendor — see the shipped references for the full pattern, including chunking, auth
token caching, and idempotency-key passthrough where a vendor supports it:
...
`@stackbase/auth` needs external systems too: it sends OTP/magic-link/verification emails, and it
...
`@stackbase/notifications` in your project, auth's verification/reset/magic-link/OTP emails
...
own separate auth service) rather than doing an OAuth redirect through stackbase, that's the `jwt`
...
token's signature, issuer, audience, and expiry — once, at sign-in, not on every request. A
verified token is then exchanged for a stackbase session the same way an OAuth login is, through
the shared identity-resolution path both transports funnel into. The same non-loopback-`http://`
guard from OAuth applies here too, on the JWKS URL.
...
Which adapter is active is decided purely by configuration — set `STACKBASE_STORAGE_BUCKET` (or
...
- **Presigned** (S3/R2): the client `PUT`s the bytes straight to the bucket, bypassing the engine
  entirely — better for large files and lower server load. Since the engine never sees the bytes go
  by, the client makes one extra call afterward — `POST` to a `confirmUrl` — to tell the engine
  "the upload finished, flip this file to ready."
...
once and pointing the storage component at it — same recipe as a custom `DatabaseAdapter`, see
[Custom storage adapter](/docs/contributing/extending/storage-adapter). For the end-user-facing
upload/download flows, capability tokens, and reaper behavior, see
...
Once you've internalized that shape, adding a new vendor anywhere in stackbase is always "write a
