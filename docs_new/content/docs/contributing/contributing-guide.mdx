...
Merged on a branch, never committed straight to `main`.
...
<Callout type="warn" title="CLA or DCO required, day one">

A CLA or DCO is required on every contribution, from the start. This exists to preserve the
project's ability to relicense or dual-license code later (for example, to keep the `ee/` split
clean). It's impossible to add retroactively: it has to be in place before a contribution lands,
not requested after. Expect to sign it as part of your first PR.
