...
This page is for people working on stackbase's own codebase, not people building an app with
...
- **Bun >= 1.2.** Bun is both the package manager and the runtime for the day-to-day workflow
  (`bun install`, `bun run build`, `stackbase dev`). Install it from [bun.sh](https://bun.sh) if
  you don't have it.
...
When package A depends on package B (say, `@stackbase/cli` depends on `@stackbase/values`), A's
...
`bun run --filter @stackbase/cli test`, your edit is invisible. Nothing breaks, nothing updates.
The test just silently keeps running against whatever was in `dist/` before your edit. You have
to `bun run build` (or `bun run --filter @stackbase/values build`) first to refresh `dist/`.
...
2. It watches the `convex/` folder (`--dir convex`) for function and schema changes.
...
If you only changed a function in `convex/` you don't need to do anything else. The watch loop
...
a **real** `stackbase dev` (or `stackbase serve`) server, the actual CLI entrypoint an end user
...
<Callout title="Any cross-package feature must be proven through the real CLI server">
  ...
  and never wired through `stackbase dev`/`serve` the way a real user's request would travel, has
...
documentation (this site): how someone *uses* stackbase. `docs/dev/` and `docs/superpowers/` are
internal engineering docs (specs, architecture decisions, plans): how stackbase itself is
