...
description: "Clone, install, build, test, and the inner loop for working on stackbase itself."
---

{/* diataxis: how-to */}

This page is for people working on stackbase's own codebase — not people building an app
with it. If you're building an app, you want the
[Quickstart](/docs/get-started/quickstart) instead.

Everything below is real: every command is copied from an actual `package.json` or
`turbo.json` in the repo, not aspirational.

## Prerequisites

- **Bun >= 1.2.** Bun is both the package manager and the runtime for the day-to-day
  workflow (`bun install`, `bun run build`, `stackbase dev`). Install it from
  [bun.sh](https://bun.sh) if you don't have it.
- **Node >= 22** is a fully-supported *target* for the published packages — end users can run
  stackbase on Node — but as a contributor you develop and run the tooling on Bun.

<Callout title="Why Bun for development but Node as a target?">
  Stackbase's engine is written to be runtime-agnostic behind small seams (like the storage
...
(`<your-fork-url>` is a placeholder — clone whichever fork/remote you're actually working from.)
...
bun run --filter @stackbase/values typecheck
```

There's also a separate end-to-end test command, covered in its own section below because it
has a different rule attached to it:
...
flowchart LR
    ...
    C --> D["stackbase dev on an example"]
    D --> A
...
When package A depends on package B (say, `@stackbase/cli` depends on `@stackbase/values`),
A's tests resolve B through B's **built** `dist/` output — the same way a real npm consumer
would — not through B's `src/`.

That means: if you edit `packages/values/src/index.ts` and immediately run
`bun run --filter @stackbase/cli test`, your edit is invisible. Nothing breaks, nothing
updates — the test just silently keeps running against whatever was in `dist/` before your
edit. You have to `bun run build` (or `bun run --filter @stackbase/values build`) first to
refresh `dist/`.

<Callout title="git checkout won't save you either" type="warn">
  `dist/` is gitignored. If you're debugging something that seems to have "reverted itself,"
  a `git checkout` on the source doesn't touch the stale `dist/` sitting on disk — you still
  need to rebuild.
</Callout>
...
**under Node** — `globalThis.Bun` is `undefined` inside those tests. Two consequences:
...
- The actual Bun runtime path is comparatively under-tested by the shared suite; it gets its
  own separate smoke test instead (for example, `docstore-sqlite` has a `bun run` smoke test
  that only runs on Bun).
...
The fastest way to see whether your change actually works — not just whether it typechecks —
is to run it inside a real example app.
...
```bash
bun ../../packages/cli/dist/bin.js dev --dir convex --web web --port 3210
```

Here's what happens when it starts:

1. It boots the **embedded runtime** — an in-process engine (storage + transaction manager +
   query engine + WebSocket sync server) — all in one Bun process, no separate database to
   start.
2. It watches the `convex/` folder (`--dir convex`) for function/schema changes.
...
If you only changed a function in `convex/` you don't need to do anything else — the watch
loop picks it up. If you changed a package under `packages/` that the example depends on,
remember gotcha #1: rebuild that package first, or the example keeps running the old code.
...
[Local development](/docs/deploy/local-dev) for the end-user-facing version of this
workflow.

## E2E tests and the cardinal testing rule

`packages/cli/test/*-e2e.test.ts` are a different animal from ordinary unit tests: each one
boots a **real** `stackbase dev` (or `stackbase serve`) server — the actual CLI entrypoint an
end user runs — and drives it over a real WebSocket or HTTP connection.
...
<Callout title="Any cross-package feature must be proven through the real CLI server">
  A mechanism that only passes in isolation — a unit test exercising some internal class
  directly, never wired through `stackbase dev`/`serve` the way a real user's request would
  travel — has repeatedly hidden real bugs in this project's history. Things like: a feature
  built correctly but never actually plumbed into the command that's supposed to use it, or a
  privilege check that only fires on one of two code paths into the same functionality.
  If your change crosses a package boundary, it needs an E2E test alongside its unit tests, not
  instead of them.
</Callout>
...
[the architecture overview](/docs/contributing/architecture/system-design)) gets a design spec under
`docs/superpowers/specs/` — reviewed and approved — before any implementation plan or code.
...
inference, and how fast `stackbase dev` starts up are treated as core product surface. If a
...
documentation (this site) — how someone *uses* stackbase. `docs/dev/` and
`docs/superpowers/` are internal engineering docs (specs, architecture decisions,
plans) — how stackbase itself is *built*. A change to one should never leak into the other.
...
(`packages/docstore-sqlite`, `packages/docstore-postgres`, ...) and into engine code is
treated as a design bug, not a style nit — see
[the storage architecture](/docs/contributing/architecture/storage) for why this boundary
matters.
...
| A new database backend | A new adapter package (implementing the same `DatabaseAdapter` interface as `docstore-sqlite`/`docstore-postgres`) |
...
- [The monorepo](/docs/contributing/codebase/monorepo) — a tour of the packages, components,
  and how they depend on each other.
- [Architecture overview](/docs/contributing/architecture/system-design) — the
  reactive-transaction core and the tiered architecture.
- [Building components](/docs/contributing/extending/custom-component) — the seam new opt-in
  features are built on.
- [Contributing guide](/docs/contributing/contributing-guide) — how to actually propose and
  land a change (PRs, CLA/DCO).
