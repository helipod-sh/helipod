...
real MVCC SQLite storage, the real single-writer OCC transactor, the real query engine, and (if you
compose them) real component drivers. There's no mocked `ctx.db` and no simulated reactivity:
`t.subscribe` drives the actual client → sync protocol → `SubscriptionManager` → engine path over an
in-process loopback connection. A passing test here is a claim about the shipped engine's actual
behavior, not about a stand-in for it.

This page is the complete reference for the harness: every option `createTestStackbase` takes, every
method on the object it returns, how to test reactivity and time-based (scheduler/cron) code, how to
compose components, and the specific, enumerated places behavior diverges from Convex's testing
tools.
...
`createTestStackbase` is **async** — always `await` it (it boots a real runtime: schema setup,
component composition, driver `start()`). Always call `t.close()`, in a `try`/`finally` or an
`afterEach` — it tears down the in-memory database, stops component drivers, and removes the temp
directory backing file storage. Every call runs under plain vitest; there's no special CI setup
beyond what your monorepo already has for the rest of its tests.
...
Keys from `import.meta.glob` come back with leading path segments (`./convex/messages.ts`) — the
harness normalizes them (stripping `./`/`../` prefixes, a leading `convex/` root, and the file
extension) to the same `module:export` path your codegen'd `api`/bare-string refs already use, so a
glob-sourced module registers under the exact path an explicit `{ "messages.ts": messages }` entry
would.
...
- **`"auto"` (default)** — resolve the schema from whatever `schema.ts` entry is in `modules` (its
...
- **`false`** — no schema at all (an empty schema — only `_storage`, see below, exists).
...
injected into the composed schema unconditionally — file storage is an always-on core feature in
the harness exactly as it is in production, never opt-in.

### `components`

If your app opts into a component in `stackbase.config.ts` (the scheduler, workflow, triggers,
notifications, auth, …), list it the same way here — the harness composes it through the same
...
rather than errors — there's no driver to drive, so the harness treats "nothing scheduled" the same
as "nothing composed."
...
harness's own internal clock would have no effect on a caller-supplied `now`. Omit `opts.now`
(the default) to get the harness-owned virtual clock those two methods drive.

### `store`

The document store to run the real engine against. Defaults to an in-memory `SqliteDocStore` over
`NodeSqliteAdapter` (`:memory:`). Pass an alternative — for example a `PostgresDocStore` — to
exercise the exact same conformance assertions against a different backend:
...
The store must start empty — the harness calls its `setupSchema()` and owns its lifecycle from
there, closing it in `t.close()`. This is how Stackbase's own suite proves read-your-own-writes
parity on Postgres (`packages/docstore-postgres/test/ryow-runtime.test.ts`), not a hypothetical — the
...
Anywhere a `ref` is expected, both a `"module:fn"` string and a typed `FunctionReference` (your
codegen'd `api`/`internal`, or `anyApi` cast when you don't have generated types in the test) work
interchangeably — they resolve to the exact same dispatchable path:
...
you'll use for the overwhelming majority of assertions: call a mutation, then query the result.

### `t.run(fn)`

Runs `fn` with a full, privileged database-writer `ctx` — the engine's real `MutationCtx` — inside
one real transaction, bypassing the public function-dispatch gate entirely. It's backed by an
internal `_test:_run` system mutation whose handler invokes whatever callback you last handed it, so
you get a genuine writer context without having to declare an app-level mutation just for test setup:
...
HTTP handler dispatches to an `httpAction`, returning a `Response` — a plain 404 for an unmatched
method+path, never a throw for that case:
...
Identity threading: the calling view's identity (set via `withIdentity`) is passed as the
httpAction's `ctx` identity, taking precedence over the request's own `Authorization` header if both
are present. If no view identity is set, the request's raw `Authorization` header is used as a
fallback and Bearer-stripped (`Bearer abc123` → `abc123`; anything else → `null`) — the same
...
`ctx.auth`, or any component that reads `cctx.identity` off the composed context — **never** a bare
...
`t.subscribe(ref, args)` opens a live subscription over the **real** client → sync protocol →
`SubscriptionManager` → engine invalidation path, via an in-process loopback connection
(`runtime.connect()` + a loopback transport) — the same wiring `examples/chat`'s own test suite
...
`value()` returns the latest pushed value (`undefined` until the first compute lands — subscribing
and the first push are asynchronous, so wait for a change before asserting on it in a fast test).
`onChange` registers a listener fired on every reactive re-run (including the first) and returns a
remover. `unsubscribe()` tears down the underlying query subscription.
...
`withIdentity` views (v1 limitation: `subscribe` always uses the base, no-identity client regardless
of which view it's called from — there is no per-identity subscription yet). It's built lazily on
first use and closed in `t.close()`, before the runtime's drivers are stopped, so no loopback
session ever outlives the runtime it's connected to.

A subscription also fans out correctly when the write comes from a **scheduled** mutation, not just
a directly-called one — a `ctx.scheduler.runAfter(...)` job's write, once it commits, invalidates a
live subscription exactly the same way a direct `t.mutation(...)` call would.

### Time control: `t.finishScheduledFunctions()` / `t.advanceTimers(ms)`

Deterministic time control for `@stackbase/scheduler` jobs and crons — no real timers or sleeps
involved. Both are driven by the harness's own virtual clock (starting at a fixed epoch millisecond,
mutable only through these two calls) and both are clean no-ops if no `@stackbase/scheduler`
component was composed via `opts.components` — there's simply nothing to drive.
...
- **`advanceTimers(ms)`** advances the clock by exactly `ms` and drives one scheduler-driver pass. It
  is the one-shot counterpart to `finishScheduledFunctions` — mirroring a real fake-timer
  ...
  composed — advancing time is a general harness primitive, not scheduler-specific.
- **`finishScheduledFunctions()`** repeatedly advances the virtual clock in large steps and drives a
  ...
  outstanding (`"pending"`/`"inProgress"`) — draining every currently- and eventually-due job,
...
- **Both throw if `opts.now` was supplied** to `createTestStackbase` — with a custom clock, the
  caller owns time, and the harness has no internal clock left to mutate. Omit `opts.now` (the
  default) to use time control.
...
Always call this — in a `try`/`finally` or an `afterEach`. It closes the shared loopback
subscription client (before stopping drivers, so no connection races driver shutdown), stops every
component driver, closes the underlying document store, and removes the temp directory backing file
storage. Skipping it doesn't corrupt any *other* instance (see Isolation below), but it will leak
resources across a long test run.

## Isolation

Every `createTestStackbase()` call is a fully independent backend: its own SQLite `:memory:`
database (or its own `store`, if supplied), its own temp file-storage directory, its own set of
component drivers. Two instances in the same test file never see each other's writes:
...
This holds under repeated create/close cycles too — creating and closing dozens of instances in a
...
Stackbase's own repository ships a **conformance suite** — a large body of tests, all written
against `@stackbase/test`, whose entire purpose is to pin down and regression-guard the shipped
engine's actual behavior across ten behavioral areas: db CRUD, index reads, pagination, validators,
identity, scheduler, ids, errors, reactivity, and the HTTP router. Every assertion in it runs through
the same harness this page documents — there's no separate, lower-fidelity mechanism-only test path.
...
- **Range-precise reactivity invalidation** — a write outside a subscribed index range genuinely
  does not re-fire that subscription (the core "range, not table" invalidation guarantee has an
  actual regression net, not just a design doc asserting it).
...
If you're porting tests written against Convex's `convex-test`, these are the specific, enumerated
places behavior diverges — each one was discovered by writing the conformance suite against real
Convex-derived expectations and finding the actual shipped behavior differs:

**1. Identity is a raw string token, not a JWT-claims object.**
...
The token is surfaced to function code only through a context provider (e.g. `@stackbase/auth`'s
`ctx.auth`, or a component reading `cctx.identity`) — there is no bare `ctx.identity` populated from
JWT claims on a plain UDF context. What the token resolves to is entirely up to your composed auth
component.

**2. Index reads use a chained query builder, not `.withIndex(cb)`.**
...
**3. Pagination returns a different result shape.**
...
`true` when the page's underlying scan hit `maxScan` before filling `pageSize` — a signal that more
matching rows may exist beyond what this page found.

**4. There is no `ctx.db.patch`.** A partial update is a read, a spread-merge, then a full
...
**5. Schema document validation is runtime-enforced.** An insert or `replace` whose document is
wrong-typed, has an extra field, or is missing a required field is rejected at write time with a
`DocumentValidationError` ("document in "&lt;table&gt;" does not match schema: ..."), not silently
accepted:
...
- **A query attempting a write** throws `ForbiddenOperationError` (`code: "FORBIDDEN"`) — but the
  load-bearing guarantee is structural, not just a guard: a query's `ctx.db` has no `insert`/
  `replace`/`delete` methods at all (`typeof ctx.db.insert === "undefined"`), so ordinary app code
  can't reach the write path regardless of the kernel-level check.
...
- **A mutation that writes, then throws, rolls back the entire transaction** — including the
  pre-throw write. There's no partial-commit state to clean up in a `finally`; assert against
  `t.run` or a follow-up query to confirm nothing persisted.
...
reactivity logic — it's fast (in-memory, no process boundary) and exercises the real engine. For
**transport- or process-level** behavior it structurally can't reach — hot reload, the raw wire
protocol, Docker boot, `stackbase deploy`'s live hot-swap, cross-runtime (Bun vs. Node) behavior —
start a real `stackbase dev`/`serve` as a child process and drive it with `@stackbase/client` over a
real WebSocket, or `POST /api/run` over HTTP. This is exactly the pattern Stackbase's own end-to-end
tests use — see `packages/cli/test/*-e2e.test.ts` in the repo for worked examples (scheduler,
actions, workflow, `httpAction`s, deploy, storage, and more, each proven against the real shipped
CLI entrypoint rather than a mechanism-only unit test).

There is no `stackbase run` command and no separate `createStackbase` testing API — use `@stackbase/test`
for in-process tests, and `POST /api/run` / the dashboard's function runner / the client SDK for
driving a real running server.

## See also

- [Client SDK](/docs/client/client-sdk) — the same `useQuery`/`useMutation` hooks a test's
  `t.subscribe` stands in for.
- [Actions](/docs/core-concepts/actions) and [Scheduled Functions](/docs/core-concepts/scheduled-functions)
  — what `t.action` and `t.finishScheduledFunctions`/`t.advanceTimers` are testing.
