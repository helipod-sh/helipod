...
Default is off. A sharded fleet already spreads commits across per-shard connections and gets no
extra benefit (or extra risk: the idle-frontier closer treats a shard with an in-flight batch as busy
and skips it) from also batching within a shard.
