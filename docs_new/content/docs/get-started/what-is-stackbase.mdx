...
Your functions live in a `convex/` folder. Here's a query that lists messages and a mutation that
adds one:
...
- **Convex.** Same reactive model and the same style of functions, but you self-host it and your data
...
- **Firebase and Supabase.** Both give you realtime, but through security rules or a stack of separate
  services around a database. stackbase gets its live updates from your own functions, in one process.
- **SpacetimeDB.** Its functions are written in Rust or C#. stackbase's are plain TypeScript, and it
...
- [Quickstart](/docs/get-started/quickstart): get a backend running and watch it update live.
- [How it works](/docs/get-started/how-it-works): the reactive model, explained.
- [Migrate from Convex](/docs/reference/migrate-from-convex): bring an existing app over.
