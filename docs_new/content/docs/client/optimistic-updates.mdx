...
`placeholderId`/`now` are covered in full detail further down this page, in the purity rules section.
Always use them instead of `crypto.randomUUID()`/`Date.now()`.
...
<Callout type="warn" title="Don't call crypto.randomUUID() or Date.now() inside an updater">

Either one mints a fresh value on every replay. React, keying off it, sees a brand-new row instead
of the same optimistic row re-rendering: remounts, list-key churn, and lost input focus, on every
unrelated server push while your mutation is pending.
