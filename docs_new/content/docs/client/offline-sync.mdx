...
It's the default a client with no `outbox` option effectively behaves as (byte-for-byte, since none
of this machinery's runtime branches are exercised at all). It's also the right tool for two
...
(this is the "S4 park-swap" arming). Before that first successful handshake in a tab session, a
...
4. Each chunk's `MutationResponse`s settle per entry. `applied` dequeues and resolves. A **coded**
   (terminal, server-recorded) failure settles the promise as a rejection, and by default the
   drain keeps going past it (see [poisonPolicy](#poison-handling-poisonpolicy)). A **codeless**
   (transient/infra) failure backs off (jittered exponential, capped at 30s) and resends starting
   from the failed unit: every unit *after* it in that chunk got no response either, and re-sends
   too.
...
<TypeTable
  type={{
    ...
    getAuthToken: {
      description:
        "A Service Worker has no access to your app's in-memory auth state; this reads from wherever the SW itself can reach it (IndexedDB, a Cache entry). Providing and refreshing that store is your app's job.",
      type: "() => Promise<string>",
    },
    poisonPolicy: {
      description:
