...
`@stackbase/client` is the framework-agnostic reactive client: a transport, a `StackbaseClient` on
top of it, and (via `@stackbase/client/react`) hooks that subscribe a component to a query and
re-render it on every push. This page covers the whole client surface — construction, transports,
every `StackbaseClient` method, the React hooks, the wire lifecycle, auth, typed codegen, and error
handling. For the deep dives on the two biggest features built on top of this client, see
[Optimistic updates](/docs/client/optimistic-updates) and [Offline sync](/docs/client/offline-sync).
...
new StackbaseClient(transport, {
  ...
  gateTimeoutMs: 5000,              // internal reconcile-gate timeout (rarely overridden)
});
```
...
`onClientReset`, `onMutationFailed`, `optimisticUpdates`) is documented in full in
[Offline sync](/docs/client/offline-sync). A client constructed with no `outbox` behaves exactly as
if none of this existed.

## Transports

The client logic is transport-agnostic. There are two:

### `loopbackTransport(connection)`

For an embedded/in-process connection — no network hop, used when the client and the engine run in
the same process (tests, `@stackbase/test`, single-process deployments). It wraps anything shaped
like `{ send, onMessage, close }` and never reconnects (`onReopen` is simply absent — the client
treats a transport without it as one that doesn't support reconnection).

### `webSocketTransport(url, options?)`

For a real deployment, talking to `stackbase dev`/`stackbase serve` over the network:
...
|---|---|---|
| `reconnect` | `true` | Reconnect automatically after a disconnect. `{ reconnect: false }` restores a terminal-on-close transport — one dead socket ends the session, byte-for-byte the old (pre-reconnect) behavior. |
| `initialBackoffMs` | `300` | Base delay before the first reconnect attempt. |
| `maxBackoffMs` | `30_000` | The backoff cap (~30s). |
| `createWebSocket` | `(url) => new WebSocket(url)` | Test/embedding seam — how to construct the underlying `WebSocket`. |

## Reconnection, in detail

**The WebSocket transport reconnects by default.** This is the behavior an app gets for free unless
it opts out with `{ reconnect: false }`.

### Backoff schedule

Each reconnect attempt waits `reconnectDelayMs(attempt, initialBackoffMs, maxBackoffMs)` — **equal
jitter**: half the exponential delay, plus up to another half at random. This never produces a zero
wait (avoiding a thundering-herd reconnect storm the instant a server comes back) and is capped at
`maxBackoffMs`:
...
`onReopen` fires once per successful **reconnect** — never for the very first connect. When it
fires, the client runs a fixed sequence, in order:

1. **Replay `SetAuth`** — if `setAuth(token)` was ever called, the same token is resent first, so
   every subscription that follows re-runs under the right identity.
2. **Resubscribe every live query** — every subscription the app still holds open is re-sent as a
   fresh `ModifyQuerySet`, and the server's reply is adopted as a brand-new baseline regardless of
   version (there's no way to know what was missed while disconnected, so the client doesn't try to
   diff across the gap — it re-establishes from scratch). If a query result hasn't changed since
   disconnect, the server can answer with a lightweight `QueryUnchanged` instead of resending the
   full value — see [Subscription lifecycle](#subscription-lifecycle-and-wire-messages) below.
3. **FIFO-flush only `unsent` mutations** — any mutation that was queued because the socket was down
   when it was called goes out now, oldest first, reusing its **original** `requestId` so the
   promise created at the original `mutation()` call site is the one that eventually resolves.

An `inflight` mutation — one whose `Mutation` frame had already been sent when the socket dropped —
is a different story: **its outcome is genuinely unknowable**. The server may or may not have
received and committed it before the drop. Without a durable outbox configured, that promise
**rejects** with `MutationUndeliveredError` rather than being blindly resent (a resend could
double-apply a mutation that actually succeeded). With a durable outbox configured and armed (see
[Offline sync](/docs/client/offline-sync)), the entry instead **parks** and a later drain resolves
it exactly once via server-side dedup.
...
try {
  ...
  if (err instanceof MutationUndeliveredError) {
    // the connection dropped before we learned whether this committed — without an outbox,
    // it's on the app to decide whether re-sending is safe for this particular mutation.
  }
}
```
...
If the very first connection attempt fails before ever opening (constructed while offline, or a
network blip on load), the transport queues frames until a socket does open — and when one finally
does, it runs the **same** reopen sequence (`SetAuth` replay, resubscribe, flush) as a mid-session
reconnect, even though technically no socket had "reopened" yet. From the app's point of view there
...
`useStackbaseClient()` reads it back out (and throws if called outside a `StackbaseProvider` — every
...
call directly — useful outside React, or when you need a one-shot read rather than a live
subscription.
...
(synchronously, if a cached value is already on hand) every time it changes; the optional `onError`
fires if the query's handler throws server-side. Returns an **unsubscribe** function:
...
composed value if an optimistic layer is in play), or rejects if the query throws — then
...
Use `useQuery`/`subscribe` when you want live updates; use `query` for a single read (e.g. inside an
event handler, or outside a React tree entirely).

### `client.mutation(ref, args?, opts?)`

Runs a mutation; resolves with its return value **at commit** (see
...
- **`transient: true`** — skip the durable offline outbox for **this call only**, even when an
  `outbox` is configured on the client. Meant for mutations that must never be durably replayed after
  a reload (the canonical example is an auth token refresh: blindly replaying a stale refresh token
...
Runs an action and resolves with its return value (or rejects with its error). Actions are **not
reactive** — there is no subscription, this is a single call/response:
...
`.withOptimisticUpdate(fn)` chains onto the callable `useMutation` returns, producing a **new**
callable with the updater bound (it doesn't mutate the original — `useMutation(ref)` itself stays
reusable without an optimistic update):
...
returns the same bound callable (no identity churn) as long as that reference itself is stable (e.g.
module-scoped, or memoized with `useCallback`) — a fresh inline closure every render churns by
necessity, since there's no way to detect "this is the same update" without a reference. Full
coverage — the store's `getQuery`/`setQuery`, `placeholderId()`/`now()` purity rules, and the
no-flicker reconciliation contract — is in [Optimistic updates](/docs/client/optimistic-updates).

`useAction` mirrors `useMutation` for actions — the side-effecting functions that run outside the
transaction (`fetch`, timers, etc.):
...
Unlike `useQuery`, neither `useMutation` nor `useAction` subscribes to anything — calling the
returned function runs the mutation/action once and returns a promise.

## Subscription lifecycle and wire messages

A `subscribe()`/`useQuery()` call sends a `ModifyQuerySet` frame adding the query; unsubscribing (or
a component unmounting) sends one removing it. In between, the server pushes `Transition` frames —
batches of **modifications**, one per changed subscription, bracketed by a version pair so the
client can detect a dropped frame and resync from scratch rather than silently miss an update. Each
...
None of this is something app code touches directly — it's the mechanism `useQuery`'s live updates
and reconnect's bandwidth savings are built on. The one part apps care about at the API level:
`onError` on `subscribe()` fires exactly once per `QueryFailed`, and the last delivered value is
never cleared out from under an `onUpdate` listener just because a later run failed.
...
Sending `SetAuth` causes the server to **re-run every live subscription** under the new identity —
any query whose result depends on `ctx.auth` (or on an identity-scoped `db.get`) re-answers with
whatever it evaluates to for the new caller, exactly like a write intersecting its read set. This is
why the reconnect sequence replays `SetAuth` *before* resubscribing (see
...
identity fingerprint from the stable session id rather than the rotating access token, so a mid-drain
rotation never orphans queued offline mutations. See [Auth](/docs/components/auth) for the full API.

## Typed codegen api

`stackbase codegen` generates a typed `api` object from your `convex/` functions — that's what makes
...
This is what makes `useQuery(api.messages.list, { conversationId })`'s second argument
type-checked and its return value typed, with zero manual annotation. A function with no `returns`
validator resolves to `any` for its return type — the args side is always typed, since `args` is far
more commonly declared. Add a `returns` validator to narrow it (see
...
|---|---|---|
| `MutationUndeliveredError` | A mutation's promise, on reconnect | The socket dropped after the `Mutation` frame was sent but before a response arrived, with no durable outbox to hand it off to. The outcome is genuinely unknown — resending blindly could double-apply it. |
...
try {
  ...
  if (err instanceof OutboxOverflowError) {
    ...
    // this specific in-flight mutation's fate is unknowable after a reset — decide whether to retry.
...
For a durable outbox, a mutation can fail **terminally** with no live promise to reject — the call
that originally enqueued it may be long gone (a prior page load, a retried entry, or a failure
discovered already-recorded when the client starts up). `onMutationFailed`, passed at client
construction, is how an app hears about those:
...
`usePendingMutations()` returns a live, reactive snapshot of the durable outbox — `[]` with no
`outbox` configured, and forever after:
...
harmless no-ops otherwise — `retry()` re-enqueues under a **fresh** `(clientId, seq)` rather than
reviving the old record (a seq is never reused once minted). The hook re-reads on every local outbox
change *and* on a cross-tab `BroadcastChannel` nudge, so a second browser tab's queued mutations show
up here too. The underlying client methods, `client.pendingMutations()` and `client.pendingSummary()`
(count + oldest-age, for a "changes may be lost soon" banner), are documented alongside the full
...
[`@stackbase/notifications`](/docs/components/notifications)' reactive in-app inbox — zero
per-app codegen required, since these hooks call the component's well-known function paths directly:
...
`unregisterForPush` are plain async functions (not hooks — registration happens once at boot, not on
every render) for wiring a device's push token. Full server-side setup — channels, providers, topics,
digests — is in [Notifications](/docs/components/notifications).

## Ephemeral broadcasts

Presence/typing-indicator style events that don't need durability or reactivity — they bypass the
...
arrives — i.e. at commit, once the write is durable — carrying the commit's timestamp. This is a
deliberate divergence from `convex-js`, whose mutation promise resolves earlier, at what Stackbase
calls the optimistic reconciliation "gate": the moment the client's own reactive feed has observed
the write, not merely been told it succeeded. If you're migrating from Convex and relied on that
earlier resolution timing (e.g. reading freshly-committed state synchronously right after `await
mutation(...)`), note that here the promise's resolution and the query re-render triggered by the
same write can arrive in either order relative to each other — see
[Optimistic updates](/docs/client/optimistic-updates#the-promise-resolves-at-commit-not-at-the-flicker-free-gate)
for exactly how the two are sequenced and why.
